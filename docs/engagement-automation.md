...
| not replied, ordinary step | Render the template, dispatch `send_private_dm`, log `FOLLOWUP`, mark `sent`, and enqueue the next step — after two gates (issue #2115). **Spacing:** a successful `DM`/`FOLLOWUP` log row to the same recipient inside `FOLLOWUP_MIN_SPACING_HOURS` (48) defers the row in place (`defer_followup`, stays `pending`) until the window has passed, because a step's `delay_hours` defaults to 24 and put two near-identical DMs in one inbox on consecutive days; an unreadable log reads as "just touched" and defers. **Refusal:** the body is graded by the same `outbound_refusal_reason` `send_dm_now` applies, BEFORE dispatch — a refused body logs `FOLLOWUP`/`failure` with `Refused: <reason>`, marks `failed`, sends nothing and still enqueues the next step. Only dispatched bodies count toward the returned "Sent N" |
...
### An approved DM is sent near its slot, one at a time (`auto_check_scheduled_dms`, issue #2103)

Every approved `scheduled_dms` row — nurture drafts included — leaves through the
`auto_check_scheduled_dms` scanner and `send_scheduled_dm`. Two rules keep a backlog from leaving
as a burst (five drafts approved for 09-07..09-13 went out 3-9 days late within two minutes):

- **Staleness.** A DM more than `SCHEDULED_DM_STALE_DAYS` (default 2) past its `scheduled_time`

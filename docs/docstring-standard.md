...
**The lint gate could not fail.** The old `Test Suite` workflow (`.github/workflows/test.yml`, since
retired — it duplicated the unit and integration lanes 3× per PR while gating nothing; the nightly
`slow-tests.yml` took over the only work it was uniquely doing) ran `ruff check` under

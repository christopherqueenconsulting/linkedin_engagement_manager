...
set -uo pipefail
cd "$(dirname "$0")/.." || exit 2
# A private file, not a fixed /tmp path: agents on one box run this concurrently (the unit tests
# drive it too), and a shared file lets one run grade another's output.
out="$(mktemp)" || exit 1
trap 'rm -f "$out"' EXIT
set +e
poetry run ruff check src/ tests/ --output-format=concise > "$out" 2>&1
rc=$?
set -e
if [ "$rc" -gt 1 ]; then
...
count="$(grep -cE '^[^ ].*:[0-9]+:[0-9]+: ' "$out" || true)"
: "${count:=0}"

...
import sys, json; print((json.load(sys.stdin).get('pr') or {}).get('body') or '')" 2>/dev/null)"
  ...
  # POSTHOG_PERSONAL_API_KEY is deliberately NOT sourced any more — it was revoked on 2026-08-31,
  # so sourcing it would hand this lane a dead credential; posthog_keys.py still names it as the
  # fallback for environments that have one.
  set -a; source <(sudo -n grep -E "^(BENCHMARK_[A-Z_]+|POSTHOG_API_KEY|POSTHOG_HOST|POSTHOG_BENCHMARK_API_KEY|POSTHOG_PROJECT_ID)=" /opt/lem/.env) 2>/dev/null; set +a

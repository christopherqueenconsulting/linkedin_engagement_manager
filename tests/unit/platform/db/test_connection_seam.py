...
class TestOneCanonicalTarget:
    ...
    def test_no_test_patches_the_re_export(self):
        """Patching `cqc_lem.utilities.db.get_db_connection` binds the facade and reaches nothing."""
        offenders = []
        for p in pathlib.Path("tests").rglob("*.py"):
            if p.name == "test_connection_seam.py":
                continue
            t = p.read_text(errors="ignore")
            if re.search(r'["\']cqc_lem\.utilities\.db\.get_db_connection["\']', t):
                offenders.append(str(p))
            if re.search(r'patch\.object\(\s*db\s*,\s*["\']get_db_connection["\']', t):
                offenders.append(str(p))
        assert offenders == [], (
            f"patch {_CANONICAL} instead — these target the re-export: {sorted(set(offenders))}")

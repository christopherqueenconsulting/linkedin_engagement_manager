...
class TestAvatarRowSerialization:
    def test_full_row_round_trips(self, fake_cursor):
        row = {
    ...
    def test_unparseable_sample_json_degrades_to_empty(self, fake_cursor):
    ...
    def test_scoped_to_owner(self, fake_cursor):
    ...
    def test_error_returns_none(self, fake_cursor):
...
class TestUpdateAvatarAttributes:
    def test_normalizes_before_storing(self, fake_cursor):
    ...
    def test_unrecognized_values_are_stored_as_null_not_guessed(self, fake_cursor):
        conn, cur = fake_cursor(rowcount=1)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import update_avatar_attributes
...
class TestSetAvatarApproval:
    def test_approve_stamps_approved_at(self, fake_cursor):
        conn, cur = fake_cursor(rowcount=1)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
    ...
    def test_reject_also_deactivates(self, fake_cursor):
    ...
    def test_unknown_status_refused_without_touching_the_db(self, fake_cursor):
        conn, cur = fake_cursor()
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import set_avatar_approval
...
class TestUpdateAvatarSamples:
    def test_stores_the_paths_without_touching_the_counter(self, fake_cursor):
...
class TestSampleRenderClaim:
    ...
    def test_regeneration_claim_increments_under_the_cap(self, fake_cursor):
        conn, cur = fake_cursor(rowcount=1)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
    ...
    def test_regeneration_claim_lost_at_the_cap(self, fake_cursor):
        conn, _ = fake_cursor(rowcount=0)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import claim_avatar_sample_render
            assert claim_avatar_sample_render(3, 4, regeneration=True, max_regenerations=3) is False

    def test_first_render_claim_is_conditional_on_no_prior_render(self, fake_cursor):
        conn, cur = fake_cursor(rowcount=1)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import claim_avatar_sample_render
    ...
    def test_second_poll_mid_render_loses_the_claim(self, fake_cursor):
        conn, _ = fake_cursor(rowcount=0)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import claim_avatar_sample_render
            assert claim_avatar_sample_render(3, 4) is False

    def test_claim_error_fails_closed(self, fake_cursor):
        conn, cur = fake_cursor()
        cur.execute.side_effect = mysql.connector.Error(msg="boom")
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import claim_avatar_sample_render
    ...
    def test_release_gives_a_regeneration_back(self, fake_cursor):
        conn, cur = fake_cursor(rowcount=1)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
    ...
    def test_release_reopens_the_first_render_only_while_no_samples_exist(self, fake_cursor):
        conn, cur = fake_cursor(rowcount=1)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import release_avatar_sample_render
    ...
    def test_release_error_returns_false(self, fake_cursor):
        conn, cur = fake_cursor()
        cur.execute.side_effect = mysql.connector.Error(msg="boom")
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import release_avatar_sample_render
...
class TestAvatarPreferences:
    def test_reads_every_flag(self, fake_cursor):
    ...
    def test_missing_user_returns_all_off(self, fake_cursor):
        conn, _ = fake_cursor(fetch_one=None)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import get_avatar_preferences
            assert not any(get_avatar_preferences(3).values())

    def test_db_error_returns_all_off(self, fake_cursor):
    ...
    def test_update_only_writes_supplied_flags(self, fake_cursor):
        conn, cur = fake_cursor(rowcount=1)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
    ...
    def test_update_ignores_unknown_keys(self, fake_cursor):
        conn, cur = fake_cursor(rowcount=1)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import update_avatar_preferences
...
class TestPostAvatarFlags:
    ...
    def test_use_avatar_is_three_valued(self, stored, expected, fake_cursor):
        conn, _ = fake_cursor(fetch_one=stored)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
    ...
    def test_use_avatar_error_is_none(self, fake_cursor):
        conn, cur = fake_cursor()
        cur.execute.side_effect = mysql.connector.Error(msg="boom")
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import get_post_use_avatar
    ...
    def test_update_post_use_avatar(self, value, stored, fake_cursor):
        conn, cur = fake_cursor(rowcount=1)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
    ...
    def test_update_post_use_avatar_error(self, fake_cursor):
        conn, cur = fake_cursor()
        cur.execute.side_effect = mysql.connector.Error(msg="boom")
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import update_post_use_avatar
    ...
    def test_mark_and_read_avatar_media(self, fake_cursor):
        conn, cur = fake_cursor(fetch_one=(1,), rowcount=1)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
    ...
    def test_avatar_media_errors_are_false(self, fake_cursor):
        conn, cur = fake_cursor()
        cur.execute.side_effect = mysql.connector.Error(msg="boom")
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import mark_post_avatar_media, post_used_avatar_media
...
class TestInsertPostUseAvatar:
    @pytest.mark.parametrize("value,stored", [(True, 1), (False, 0), (None, None)])
    def test_compose_choice_is_persisted(self, value, stored, fake_cursor):

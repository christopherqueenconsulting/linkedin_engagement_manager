...
class TestGetUserAnalyticsProfile:
    def test_returns_plan_timezone_signup_and_the_survey_targeting_facts(self, fake_cursor):
        row = {
    ...
    def test_the_approval_tally_survives_a_post_moving_on(self, fake_cursor):
    ...
    def test_onboarding_completion_comes_from_activation_not_signup(self, fake_cursor):
        conn, cur = fake_cursor(fetch_one={})
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import get_user_analytics_profile
            get_user_analytics_profile(1)
    ...
    def test_selects_no_credentials(self, fake_cursor):
        conn, cur = fake_cursor(fetch_one={})
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import get_user_analytics_profile
            get_user_analytics_profile(1)
    ...
    def test_unknown_user_returns_empty_dict(self, fake_cursor):
        conn, _ = fake_cursor(fetch_one=None)
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            from cqc_lem.utilities.db import get_user_analytics_profile
            assert get_user_analytics_profile(999) == {}

    def test_db_error_returns_empty_dict(self, fake_cursor):

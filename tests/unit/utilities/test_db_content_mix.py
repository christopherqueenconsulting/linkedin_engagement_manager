...
class TestInsertPlannedPost:
    def test_persists_the_mix_class(self, fake_cursor):
    ...
    def test_unclassified_post_stores_null(self, fake_cursor):
        from cqc_lem.utilities.db import PostType, insert_planned_post
        conn, cur = fake_cursor()
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
...
class TestGetPostContentMix:
    def test_returns_the_class(self, fake_cursor):
        from cqc_lem.utilities.db import get_post_content_mix
        conn, cur = fake_cursor(fetch_one={"content_mix": "authority"})
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
            assert get_post_content_mix(9) == "authority"
        assert cur.execute.call_args[0][1] == (9,)

    def test_none_for_unclassified_or_missing_post(self, fake_cursor):
        from cqc_lem.utilities.db import get_post_content_mix
...
class TestPlannedPostsCarryTheMix:
    def test_selected_with_the_planned_rows(self, fake_cursor):
...
class TestGetContentMixCounts:
    def test_groups_by_class_and_buckets_nulls(self, fake_cursor):
        from cqc_lem.utilities.db import get_content_mix_counts
        conn, cur = fake_cursor(fetch_all=[("value", 21), ("authority", 6), ("promo", 3), (None, 4)])
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):
    ...
    def test_windows_on_days(self, fake_cursor):
        from cqc_lem.utilities.db import get_content_mix_counts
        conn, cur = fake_cursor(fetch_all=[("value", 2)])
        with patch("cqc_lem.platform.db.connection.get_db_connection", return_value=conn):

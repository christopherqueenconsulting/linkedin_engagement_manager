...
def _queue(**overrides):
    from cqc_lem.app.run_automation import _queue_artifact_delivery
...
class TestQueueArtifactDelivery:
    def test_queues_a_pending_draft_and_records_the_recipient(self):
        ...
        with patch(f"{_RA}.has_received_lead_magnet", return_value=False), \
             ...
             patch(f"{_RA}.insert_scheduled_dm", return_value=42) as ins:
            assert _queue() == 42
    ...
    def test_no_keyword_in_the_comment_delivers_nothing(self):
        with patch(f"{_RA}.insert_scheduled_dm") as ins:
    ...
    def test_keyword_match_is_case_insensitive(self):
        with patch(f"{_RA}.has_received_lead_magnet", return_value=False), \
             patch(f"{_RA}.has_open_scheduled_dm", return_value=False), \
             patch(f"{_RA}.count_scheduled_dms_created_today", return_value=0), \
             patch(f"{_RA}.record_lead_magnet_sent"), \
             patch(f"{_RA}.insert_scheduled_dm", return_value=1):
            assert _queue(comment_text="send me the audit") == 1

    def test_already_delivered_person_is_skipped(self):
        with patch(f"{_RA}.has_received_lead_magnet", return_value=True), \
             patch(f"{_RA}.insert_scheduled_dm") as ins:
            assert _queue() is None
        ins.assert_not_called()

    def test_one_open_draft_per_thread(self):
        with patch(f"{_RA}.has_received_lead_magnet", return_value=False), \
             patch(f"{_RA}.has_open_scheduled_dm", return_value=True), \
             patch(f"{_RA}.insert_scheduled_dm") as ins:
            assert _queue() is None
        ins.assert_not_called()
    ...
    def test_an_open_nurture_draft_also_blocks_it(self):
        ...
        with patch(f"{_RA}.has_received_lead_magnet", return_value=False), \
             patch(f"{_RA}.has_open_scheduled_dm", side_effect=_open), \
             patch(f"{_RA}.insert_scheduled_dm") as ins:
            assert _queue() is None
        ins.assert_not_called()
    ...
    def test_daily_cap_is_the_users_dm_cap(self):
        with patch(f"{_RA}.has_received_lead_magnet", return_value=False), \
             patch(f"{_RA}.has_open_scheduled_dm", return_value=False), \
             patch(f"{_RA}.count_scheduled_dms_created_today", return_value=5), \
             patch(f"{_RA}.insert_scheduled_dm") as ins:
            assert _queue() is None
        ins.assert_not_called()
    ...
    def test_empty_rendered_message_is_not_queued(self):
        with patch(f"{_RA}.has_received_lead_magnet", return_value=False), \
             patch(f"{_RA}.has_open_scheduled_dm", return_value=False), \
             patch(f"{_RA}.count_scheduled_dms_created_today", return_value=0), \
             patch(f"{_RA}.render_dm_placeholders", return_value="   "), \
             patch(f"{_RA}.insert_scheduled_dm") as ins:
            assert _queue() is None
        ins.assert_not_called()
    ...
    def test_failed_insert_does_not_mark_the_person_delivered(self):
        with patch(f"{_RA}.has_received_lead_magnet", return_value=False), \
             ...
             patch(f"{_RA}.count_scheduled_dms_created_today", return_value=0), \
             patch(f"{_RA}.record_lead_magnet_sent") as rec, \
             patch(f"{_RA}.insert_scheduled_dm", return_value=None):
            assert _queue() is None
        rec.assert_not_called()

    def test_a_db_error_never_breaks_the_reply_sweep(self):
        with patch(f"{_RA}.has_received_lead_magnet", side_effect=RuntimeError("db down")):
            assert _queue() is None

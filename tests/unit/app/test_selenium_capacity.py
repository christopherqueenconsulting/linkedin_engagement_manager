...
class TestGridOverlay:
    ...
    def test_the_hub_answers_to_the_standalones_name(self):
        # Callers that predate the Grid address `selenium-chrome:4444` — the tunnel's lemgrid
        # hostname, SELENIUM_HUB_HOST in the box .env, host-side tooling. They are not all in this
        # repo, so dropping the alias breaks them silently (the console 502s) rather than loudly.
        hub = _config_only(_service_block(GRID_OVERLAY, "selenium-hub"))
        aliases = hub.split("aliases:")[1].split("deploy:")[0]
        assert "- selenium-chrome" in aliases

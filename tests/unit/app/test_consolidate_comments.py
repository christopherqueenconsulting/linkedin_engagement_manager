...
class TestDeleteExtraOwnComments:
    ...
    def test_single_comment_is_noop(self):
        from cqc_lem.app.engagement.feed import _delete_extra_own_comments_on_post
...
def _run_task(*, dupes, dry_run=True, full_name="Chris Q", profile_raises=False):
    from cqc_lem.app.engagement.feed import consolidate_duplicate_comments_for_user
    driver = MagicMock()
    wait = MagicMock()
    profile = MagicMock()
    ...
    with patch(f"{_FEED}.get_duplicate_comment_posts", return_value=dupes), \
         patch(f"{_FEED}.get_current_profile", gcp), \
         patch(f"{_FEED}._delete_extra_own_comments_on_post", return_value=(2, 0 if dry_run else 1)) as dele, \

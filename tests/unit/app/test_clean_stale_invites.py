...
class TestCleanStaleInvites:
    def test_zero_allowance_opens_no_browser_and_still_reports(self):
        ...
        with patch(f"{_RA}.plan_withdrawals", return_value=_plan()), \
                patch(f"{_RA}.get_driver_wait_pair") as driver_pair, \
                patch(f"{_RA}.track_stale_invite_run") as track:
    ...
    def test_the_off_by_default_skip_is_debug_not_info(self):
        ...
        with patch(f"{_RA}.plan_withdrawals", return_value=_plan()), \
                ...
                patch(f"{_RA}.log_debug") as debug, patch(f"{_RA}.log_info") as info:
    ...
    def test_a_real_run_walks_the_page_and_quits_the_driver(self):
        ...
        with patch(f"{_RA}.plan_withdrawals", return_value=_plan(allowance=3, status="withdrew")), \
                ...
                patch(f"{_RA}.get_user_password_pair_by_id", return_value=("a@b.c", "pw")), \
                patch(f"{_RA}.login_to_linkedin"), \
                patch(f"{_RA}.withdraw_stale_invites", return_value=report), \
    ...
    def test_the_breaker_opening_mid_run_defers_rather_than_failing(self):
        ...
        with patch(f"{_RA}.plan_withdrawals", return_value=_plan(allowance=3, status="withdrew")), \
                ...
                patch(f"{_RA}.get_user_password_pair_by_id", return_value=("a@b.c", "pw")), \
                patch(f"{_RA}.login_to_linkedin", side_effect=LinkedInRateLimited("429")), \
                patch(f"{_RA}.quit_gracefully"), \
                patch(f"{_RA}.log_error") as error, \
                patch(f"{_RA}.track_stale_invite_run"):
            result = clean_stale_invites.run(user_id=5)
    ...
    def test_a_failing_walk_still_reports_and_quits(self):
        ...
        with patch(f"{_RA}.plan_withdrawals", return_value=_plan(allowance=3, status="withdrew")), \
                ...
                patch(f"{_RA}.get_user_password_pair_by_id", return_value=("a@b.c", "pw")), \
                patch(f"{_RA}.login_to_linkedin"), \
                patch(f"{_RA}.withdraw_stale_invites", side_effect=RuntimeError("boom")), \
                patch(f"{_RA}.quit_gracefully") as quit_driver, \
                patch(f"{_RA}.log_error") as error, \
                patch(f"{_RA}.track_stale_invite_run") as track:
            result = clean_stale_invites.run(user_id=5)

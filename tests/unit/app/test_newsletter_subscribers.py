...
class TestReadSubscriberCount:
    ...
    def test_reads_from_element(self):
        ...
        with patch(f"{_NL}.find_first", return_value=el), \
             patch(f"{_NL}.getText", return_value="4,560 subscribers"):
...
class TestInviteConnections:
    def test_zero_cap_no_op(self):
        from cqc_lem.app.engagement.newsletter import _invite_connections_to_newsletter
        with patch(f"{_NL}.click_first") as cf:
            assert _invite_connections_to_newsletter(MagicMock(), MagicMock(), 0) == 0
        cf.assert_not_called()

    def test_zero_when_no_invite_button(self):
        from cqc_lem.app.engagement.newsletter import _invite_connections_to_newsletter
        with patch(f"{_NL}.click_first", return_value=None):
    ...
    def test_selects_up_to_cap_and_sends(self):
        ...
        with patch(f"{_NL}.click_first", return_value=MagicMock()), \
             patch(f"{_NL}.get_elements_as_list_wait_stale", return_value=boxes):
    ...
    def test_zero_when_no_checkboxes(self):
        from cqc_lem.app.engagement.newsletter import _invite_connections_to_newsletter
        with patch(f"{_NL}.click_first", return_value=MagicMock()), \
             patch(f"{_NL}.get_elements_as_list_wait_stale", return_value=[]):
    ...
    def test_zero_when_send_button_missing(self):
        ...
        with patch(f"{_NL}.click_first", side_effect=[MagicMock(), None]), \
             patch(f"{_NL}.get_elements_as_list_wait_stale", return_value=boxes):
...
class TestTrackNewsletterSubscribers:
    ...
    def test_captures_count_without_inviting_when_opted_out(self):
        ...
        with patch(f"{_NL}.get_newsletter_settings", return_value=settings), \
             ...
             patch(f"{_NL}._read_newsletter_subscriber_count", return_value=210), \
             patch(f"{_NL}._invite_connections_to_newsletter") as invite, \
    ...
    def test_invites_within_cap_when_opted_in(self):
        ...
        with patch(f"{_NL}.get_newsletter_settings", return_value=settings), \
             patch(f"{_NL}.get_current_profile", return_value=(MagicMock(), MagicMock(), "e", MagicMock())), \
             patch(f"{_NL}._read_newsletter_subscriber_count", return_value=210), \
             patch(f"{_NL}._invite_connections_to_newsletter", return_value=25) as invite, \
             patch(f"{_NL}.record_newsletter_subscriber_stat") as rec, \
             patch(f"{_NL}.quit_gracefully"):
            result = track_newsletter_subscribers.run(user_id=1)
    ...
    def test_records_unknown_when_count_unreadable(self):
        ...
        with patch(f"{_NL}.get_newsletter_settings", return_value=settings), \
             patch(f"{_NL}.get_current_profile", return_value=(MagicMock(), MagicMock(), "e", MagicMock())), \
             patch(f"{_NL}._read_newsletter_subscriber_count", return_value=None), \
             patch(f"{_NL}.record_newsletter_subscriber_stat") as rec, \
             patch(f"{_NL}.quit_gracefully"):
            result = track_newsletter_subscribers.run(user_id=1)

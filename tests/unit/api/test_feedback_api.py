...
class TestSubmitFeedback:
    def test_persists_with_session_user_and_context(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=9), \
            ...
            resp = api_client.post("/api/feedback", json={
    ...
    def test_anonymous_submission_has_no_user_id(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id") as sess, \
             patch("cqc_lem.api.main.insert_feedback", return_value=102) as ins:
            resp = api_client.post("/api/feedback", json={"body": "love the new tabs"})
        assert resp.status_code == 200
        sess.assert_not_called()  # no token -> never hit the session table
        assert ins.call_args.kwargs["user_id"] is None

    def test_invalid_session_token_falls_back_to_anonymous(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=None), \
             patch("cqc_lem.api.main.insert_feedback", return_value=103) as ins:
            resp = api_client.post("/api/feedback", json={"session_token": "stale", "body": "hi"})
        assert resp.status_code == 200
        assert ins.call_args.kwargs["user_id"] is None

    def test_screenshot_rides_along_in_context(self, api_client):
    ...
    def test_oversized_context_is_dropped_but_screenshot_kept(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=1), \
    ...
    def test_blank_body_rejected_422(self, api_client):
        with patch("cqc_lem.api.main.insert_feedback") as ins:
            resp = api_client.post("/api/feedback", json={"body": "   "})
        assert resp.status_code == 422
        ins.assert_not_called()

    def test_over_long_body_rejected_422(self, api_client):
        with patch("cqc_lem.api.main.insert_feedback") as ins:
            resp = api_client.post("/api/feedback", json={"body": "x" * 5001})
        assert resp.status_code == 422
        ins.assert_not_called()

    def test_unknown_source_rejected_422(self, api_client):
        with patch("cqc_lem.api.main.insert_feedback") as ins:
            resp = api_client.post("/api/feedback", json={"body": "hi", "source": "carrier-pigeon"})
        assert resp.status_code == 422
        ins.assert_not_called()

    def test_alternate_source_accepted(self, api_client):
        with patch("cqc_lem.api.main.insert_feedback", return_value=106) as ins:
            resp = api_client.post("/api/feedback", json={"body": "9/10", "source": "nps"})
        assert resp.status_code == 200
        from cqc_lem.utilities.db import FeedbackSource
        assert ins.call_args.kwargs["source"] == FeedbackSource.NPS

    def test_db_failure_returns_500(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=1), \
             patch("cqc_lem.api.main.insert_feedback", return_value=None):
            resp = api_client.post("/api/feedback", json={"session_token": "tok", "body": "hi"})

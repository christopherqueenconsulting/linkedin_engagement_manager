...
class TestNewsletterSettings:
    def test_get_returns_settings(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.get_newsletter_settings",
                   return_value={"enabled": True, "cadence": "weekly"}):
            resp = api_client.get(f"/api/user/newsletter-settings?session_token={_SESSION}")
        assert resp.status_code == 200
        assert resp.json()["detail"]["cadence"] == "weekly"

    def test_put_updates(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
    ...
    def test_put_clamps_draft_config(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.update_newsletter_settings", return_value=True) as upd:
            resp = api_client.put("/api/user/newsletter-settings", json={
                "session_token": _SESSION, "enabled": True,
    ...
    def test_put_clamps_draft_config_low(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.update_newsletter_settings", return_value=True) as upd:
            resp = api_client.put("/api/user/newsletter-settings", json={
                "session_token": _SESSION, "enabled": True,
    ...
    def test_put_enabled_triggers_queue_topup(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
    ...
    def test_put_disabled_does_not_trigger_topup(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.update_newsletter_settings", return_value=True), \
             patch("cqc_lem.app.run_scheduler.generate_newsletter_drafts_for_user") as task:
            resp = api_client.put("/api/user/newsletter-settings", json={
    ...
    def test_put_clamps_max_invites_high(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.update_newsletter_settings", return_value=True) as upd:
            resp = api_client.put("/api/user/newsletter-settings", json={
                "session_token": _SESSION, "enabled": True,
    ...
    def test_put_clamps_max_invites_low(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.update_newsletter_settings", return_value=True) as upd:
            resp = api_client.put("/api/user/newsletter-settings", json={
    ...
    def test_401(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=None):
            resp = api_client.get("/api/user/newsletter-settings?session_token=bad")
        assert resp.status_code == 401


class TestNewsletterSubscribers:
    def test_get_returns_history_and_latest(self, api_client):
        ...
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
            ...
            resp = api_client.get(f"/api/user/newsletter-subscribers?session_token={_SESSION}")
        assert resp.status_code == 200
        detail = resp.json()["detail"]
        assert detail["latest"] == 130 and detail["history"][0]["subscriber_count"] == 130

    def test_get_includes_owned_asset_delivery_attribution(self, api_client):
    ...
    def test_401(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=None):
            resp = api_client.get("/api/user/newsletter-subscribers?session_token=bad")
        assert resp.status_code == 401


class TestNewsletterDraft:
    def test_get_returns_editions_and_next_publish(self, api_client):
    ...
    def test_get_empty_when_no_editions(self, api_client):
        settings = {"publish_day": 1, "publish_hour": 9, "cadence": "weekly", "last_published_at": None,
    ...
    def test_get_401(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=None):
    ...
    def test_put_save_leaves_status_none(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.get_newsletter_edition", return_value={"id": 4, "user_id": _USER}), \
             patch("cqc_lem.api.routers.user.update_newsletter_edition", return_value=True) as upd:
            resp = api_client.put("/api/user/newsletter-draft", json={
    ...
    def test_put_forwards_scheduled_datetime(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.get_newsletter_edition", return_value={"id": 4, "user_id": _USER}), \
             patch("cqc_lem.api.routers.user.update_newsletter_edition", return_value=True) as upd:
            resp = api_client.put("/api/user/newsletter-draft", json={
    ...
    def test_put_title_edit_rebriefs_ai_cover(self, api_client):
        existing = {
            "id": 4, "user_id": _USER,
    ...
    def test_put_subtitle_edit_rebriefs_ai_cover(self, api_client):
        existing = {
            "id": 4, "user_id": _USER,
    ...
    def test_put_body_only_edit_does_not_rebrief(self, api_client):
        existing = {
            "id": 4, "user_id": _USER,
    ...
    def test_put_title_edit_on_upload_cover_does_not_rebrief(self, api_client):
        existing = {
            "id": 4, "user_id": _USER,
            "title": "Old Title", "subtitle": "Old Subtitle", "body": "Body",
            "cover_image_path": "images/newsletter_covers/5/ed4_old.png",
    ...
    def test_put_clear_fails_logs_warning_but_queues_generation(self, api_client):
        existing = {
            "id": 4, "user_id": _USER,
            "title": "Old Title", "subtitle": "Old Subtitle", "body": "Body",
            "cover_image_path": "images/newsletter_covers/5/ed4_old.png",
    ...
    def test_put_title_edit_on_ai_cover_with_no_status_does_not_rebrief(self, api_client):
        existing = {
            "id": 4, "user_id": _USER,
            "title": "Old Title", "subtitle": "Old Subtitle", "body": "Body",
            "cover_image_path": None,
            "cover_image_source": "ai", "cover_image_status": None,
        }
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.get_newsletter_edition", return_value=existing), \
             patch("cqc_lem.api.routers.user.update_newsletter_edition", return_value=True), \
    ...
    def test_put_404_when_not_owner(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
    ...
    def test_put_401(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=None):
            resp = api_client.put("/api/user/newsletter-draft", json={
...
class TestNewsletterRegenerate:
    def test_dispatches_task_with_guidance(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.get_newsletter_edition", return_value={"id": 4, "user_id": _USER}), \
             patch("cqc_lem.app.run_scheduler.regenerate_newsletter_edition") as task:
            resp = api_client.post("/api/user/newsletter-draft/regenerate", json={
    ...
    def test_404_when_not_owner(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.get_newsletter_edition", return_value={"id": 4, "user_id": 999}), \
             patch("cqc_lem.app.run_scheduler.regenerate_newsletter_edition") as task:
            resp = api_client.post("/api/user/newsletter-draft/regenerate", json={
...
class TestNewsletterCoverUpload:
    ...
    def test_upload_stores_the_cover_as_approved(self, api_client, tmp_path):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
    ...
    def test_a_rejected_image_is_a_400_and_stores_nothing(self, api_client, tmp_path):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.get_newsletter_edition", return_value=dict(_OWNED)), \
             patch("cqc_lem.utilities.newsletter_cover.assets_dir", str(tmp_path)), \
    ...
    def test_a_failed_write_leaves_no_orphan_file(self, api_client, tmp_path):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.get_newsletter_edition", return_value=dict(_OWNED)), \
             patch("cqc_lem.utilities.newsletter_cover.assets_dir", str(tmp_path)), \
    ...
    def test_replacing_a_cover_deletes_the_previous_file(self, api_client, tmp_path):
    ...
    def test_404_when_not_owner(self, api_client, tmp_path):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.get_newsletter_edition", return_value={"id": 4, "user_id": 999}), \
             patch("cqc_lem.utilities.db.set_edition_cover_image") as store:
            resp = api_client.post("/api/user/newsletter-draft/cover",
...
class TestNewsletterCoverGenerate:
    def test_dispatches_the_task(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.get_newsletter_edition", return_value=dict(_OWNED)), \
             patch("cqc_lem.app.run_scheduler.generate_newsletter_cover") as task:
...
class TestNewsletterCoverDecision:
    ...
    def test_approve_clears_the_cover_for_publish(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
            ...
            resp = api_client.post("/api/user/newsletter-draft/cover/decision", json={
                "session_token": _SESSION, "edition_id": 4, "action": "approve"})
        assert resp.status_code == 200
        assert upd.call_args[0] == (4, _USER, "approved")

    def test_remove_drops_the_row_and_the_file(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.get_newsletter_edition", return_value=dict(self._WITH_COVER)), \
             patch("cqc_lem.utilities.db.clear_edition_cover_image", return_value=True) as clear, \
             patch("cqc_lem.utilities.newsletter_cover.remove_cover_file") as rm:
    ...
    def test_404_when_there_is_no_cover(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.get_newsletter_edition", return_value=dict(_OWNED)), \
    ...
    def test_unknown_action_is_a_400(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.get_newsletter_edition", return_value=dict(self._WITH_COVER)), \
             patch("cqc_lem.utilities.db.set_edition_cover_status") as upd, \
             patch("cqc_lem.utilities.db.clear_edition_cover_image") as clear:
...
class TestNewsletterDraftCoverSurface:
    def test_queue_returns_a_url_and_never_the_filesystem_path(self, api_client):
        editions = [{"id": 4, "title": "T", "subtitle": "S", "body": "B", "status": "draft",
                     "scheduled_for": None,
    ...
    def test_settings_put_forwards_the_cover_opt_in(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.update_newsletter_settings", return_value=True) as upd:
            resp = api_client.put("/api/user/newsletter-settings", json={
    ...
    def test_cover_opt_in_defaults_off(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.update_newsletter_settings", return_value=True) as upd:
            api_client.put("/api/user/newsletter-settings", json={
                "session_token": _SESSION, "enabled": True})

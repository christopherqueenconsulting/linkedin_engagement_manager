...
class TestShippedNoticesEndpoint:
    def test_returns_the_users_notices_and_the_changelog(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=42), \
            ...
            resp = api_client.get("/api/user/shipped?session_token=tok")
    ...
    def test_requires_a_valid_session(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=None):
            resp = api_client.get("/api/user/shipped?session_token=bad")
        assert resp.status_code == 401


class TestAckEndpoint:
    def test_records_the_micro_csat_and_marks_the_notice_seen(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=42), \
             patch(f"{_DB}.get_unseen_shipped_notices", return_value=[_NOTICE]) as unseen, \
            ...
            resp = api_client.post("/api/shipped/ack", json={
    ...
    def test_a_notice_still_inside_the_csat_delay_is_not_ackable(self, api_client):

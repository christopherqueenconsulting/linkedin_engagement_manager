...
class TestPinDemotion:
    def test_a_pin_still_signs_in_an_account_with_no_strong_factor(self, api_client):
    ...
    def test_a_pin_alone_no_longer_signs_in_an_account_that_enrolled_one(self, api_client):
        with patch(f"{_AUTH}.check_auth_verify", return_value=_Allowed()), \
             patch(f"{_AUTH}.get_pin_lockout", return_value=None), \
             patch(f"{_AUTH}.hash_pin", return_value="h"), \
             patch(f"{_AUTH}.verify_pin_for_email", return_value=True), \
    ...
    def test_the_mailbox_is_still_recorded_as_verified(self, api_client):
        ...
        with patch(f"{_AUTH}.check_auth_verify", return_value=_Allowed()), \
            ...
            api_client.post("/api/auth/email/verify", json={"email": _EMAIL, "pin": "123456"})
        verified.assert_called_once_with(_UID)

    def test_the_no_mail_provider_bypass_is_gated_too(self, api_client):
        ...
        with patch(f"{_AUTH}.check_auth_init", return_value=_Allowed()), \
             ...
             patch(f"{_AUTH}.create_auth_challenge", return_value="pending-handle"), \
             patch(f"{_AUTH}.create_session") as create_session:
            resp = api_client.post("/api/auth/email/init", json={"email": _EMAIL})
        detail = resp.json()["detail"]
        assert detail["second_factor_required"] is True
        assert "session_token" not in detail
        create_session.assert_not_called()
...
class TestSecondFactor:
    ...
    def test_a_totp_code_mints_a_fully_verified_session(self, api_client):
        with self._pending(), \
            ...
            resp = api_client.post("/api/auth/second-factor/verify",
    ...
    def test_a_recovery_code_signs_in_but_does_not_unlock_the_credentials(self, api_client):
    ...
    def test_a_replayed_pending_token_is_refused(self, api_client):
    ...
    def test_a_wrong_code_creates_no_session(self, api_client):
        with self._pending(), \
             patch(f"{_AUTH}.check_auth_verify", return_value=_Allowed()), \
             patch(f"{_AUTH}.get_user_email", return_value=_EMAIL), \
    ...
    def test_a_mistyped_code_does_not_end_the_sign_in(self, api_client):
    ...
    def test_the_last_allowed_attempt_burns_the_handle(self, api_client):
    ...
    def test_a_correct_code_spends_the_handle(self, api_client):
        with self._pending(), \
             patch(f"{_AUTH}.check_auth_verify", return_value=_Allowed()), \
             patch(f"{_AUTH}.get_user_email", return_value=_EMAIL), \
             patch(f"{_AUTH}.verify_totp_code", return_value=True), \
    ...
    def test_an_unknown_method_is_not_a_way_in(self, api_client):
        with self._pending(), \
             patch(f"{_AUTH}.check_auth_verify", return_value=_Allowed()), \
             patch(f"{_AUTH}.get_user_email", return_value=_EMAIL), \
             patch(f"{_AUTH}.create_session") as create_session:
    ...
    def test_the_second_factor_step_is_rate_limited(self, api_client):
...
class TestPasskeyLogin:
    def test_begin_takes_no_email_and_returns_options(self, api_client):
        with patch(f"{_M}.webauthn_relying_party"), \
            ...
            resp = api_client.post("/api/auth/passkey/login/begin", json={})
        assert resp.status_code == 200
        assert resp.json()["detail"]["handle"] == "handle"

    def test_a_verified_assertion_mints_a_session_already_stepped_up(self, api_client):
        ...
        with patch(f"{_M}.webauthn_relying_party"), \
             ...
             patch(f"{_AUTH}.get_user_email", return_value=_EMAIL), \
             patch(f"{_AUTH}.create_session", return_value=_TOKEN) as create_session:
            resp = api_client.post("/api/auth/passkey/login/complete",
                               json={"handle": "h", "credential": {"id": "cred"}})
        assert resp.status_code == 200
        assert create_session.call_args.kwargs["verified"] is True
        counter.assert_called_once_with(5, 4)

    def test_an_unverifiable_assertion_is_a_401_and_no_session(self, api_client):
        with patch(f"{_M}.webauthn_relying_party"), \
             patch(f"{_AUTH}.consume_auth_challenge", return_value={"user_id": None,
                                                                 "challenge": "chal"}), \
             patch(f"{_M}.credential_id_from_response", return_value="cred"), \
    ...
    def test_an_unknown_credential_signs_nobody_in(self, api_client):
        with patch(f"{_M}.webauthn_relying_party"), \
             patch(f"{_AUTH}.consume_auth_challenge", return_value={"user_id": None,
                                                                 "challenge": "chal"}), \
             patch(f"{_M}.credential_id_from_response", return_value="cred"), \
    ...
    def test_a_deployment_without_a_secure_origin_says_503_not_500(self, api_client):
...
class TestEnrollment:
    def test_the_first_factor_needs_a_session_but_not_step_up(self, api_client, signed_in):
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            resp = api_client.post("/api/user/passkeys/register/begin", json={"session_token": _TOKEN})
        assert resp.status_code == 200
    ...
    def test_adding_a_SECOND_factor_is_step_up_gated(self, api_client, path, body):
    ...
    def test_a_recovery_code_session_may_still_enrol_a_replacement(self, api_client):
    ...
    def test_a_recovery_code_session_is_not_stepped_up_by_enrolling(self, api_client, signed_in):
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            resp = api_client.post("/api/user/passkeys/register/complete",
                               json={"session_token": _TOKEN, "handle": "h", "credential": {}})
        assert resp.status_code == 200
        stepped_up.assert_not_called()

    def test_a_second_authenticator_app_is_refused(self, api_client, signed_in):
    ...
    def test_a_registration_challenge_belonging_to_someone_else_is_refused(self, api_client, signed_in):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_M}.webauthn_relying_party"), \
             patch(f"{_USER}.consume_auth_challenge",
    ...
    def test_a_verified_registration_steps_the_session_up(self, api_client, signed_in):
    ...
    def test_an_unverifiable_registration_stores_nothing(self, api_client, signed_in):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_M}.webauthn_relying_party"), \
             patch(f"{_USER}.consume_auth_challenge",
                   return_value={"user_id": _UID, "challenge": "chal"}), \
    ...
    def test_totp_enrolment_returns_the_secret_exactly_once(self, api_client, signed_in):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.get_user_email", return_value=_EMAIL), \
    ...
    def test_a_wrong_totp_code_does_not_confirm_the_factor(self, api_client, signed_in):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
    ...
    def test_removing_a_factor_is_step_up_gated(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.step_up_satisfied", return_value=False), \
             patch(f"{_USER}.available_methods", return_value=["passkey"]), \
             patch(f"{_USER}.delete_auth_factor") as delete:
            resp = api_client.post("/api/user/auth-factors/delete",
                               json={"session_token": _TOKEN, "factor_id": 4})
        assert resp.status_code == 403
        assert resp.json()["detail"]["code"] == "step_up_required"
        delete.assert_not_called()

    def test_regenerating_recovery_codes_is_step_up_gated(self, api_client):
    ...
    def test_recovery_codes_are_returned_once_when_the_gate_passes(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
...
class TestStepUpGate:
    ...
    def test_the_refusal_is_403_not_401(self, api_client):
    ...
    def test_a_fresh_factor_lets_the_write_through(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.step_up_satisfied", return_value=True), \
    ...
    def test_only_the_cookie_endpoint_accepts_the_extension_scope(self, api_client):
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            api_client.post("/api/user/linkedin-cookie",
                        json={"session_token": _TOKEN, "li_at": "a" * 40})
            cookie_flag = seen["0"]
            api_client.post("/api/user/sessions/revoke",
                        json={"session_token": _TOKEN, "all_others": True})
            revoke_flag = seen["1"]
        assert cookie_flag is True
        assert revoke_flag is False

    def test_the_extension_token_is_minted_with_its_own_scope(self, api_client):
...
class TestStepUpVerify:
    ...
    def test_another_accounts_passkey_cannot_step_this_session_up(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.get_user_email", return_value=_EMAIL), \
             patch(f"{_USER}.check_auth_verify", return_value=_Allowed()), \
             patch(f"{_M}.webauthn_relying_party"), \
    ...
    def test_a_step_up_challenge_issued_for_another_account_is_refused(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.get_user_email", return_value=_EMAIL), \
             patch(f"{_USER}.check_auth_verify", return_value=_Allowed()), \
             patch(f"{_M}.webauthn_relying_party"), \
    ...
    def test_step_up_attempts_are_rate_limited(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.get_user_email", return_value=_EMAIL), \
    ...
    def test_beginning_a_step_up_without_a_passkey_is_a_400(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_M}.webauthn_relying_party"), \
             patch(f"{_USER}.get_user_passkey_credential_ids", return_value=[]):
            resp = api_client.post("/api/user/step-up/begin", json={"session_token": _TOKEN})
        assert resp.status_code == 400
...
class TestSessionRequired:
    ...
    def test_no_session_is_a_401(self, api_client, method, path, body):
        with patch(f"{_M}.get_session_user_id", return_value=None), \
             ...
             patch(f"{_USER}.delete_auth_factor") as delete, \
             patch(f"{_USER}.generate_recovery_codes") as generate:
            resp = getattr(api_client, method)(path, json={"session_token": "nope", **body})
        assert resp.status_code == 401
...
class TestFailedWrites:
    ...
    def test_a_challenge_that_cannot_be_stored_aborts_the_registration(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_M}.webauthn_relying_party"), \
             patch(f"{_USER}.get_user_email", return_value=_EMAIL), \
             patch(f"{_USER}.get_user_passkey_credential_ids", return_value=[]), \
    ...
    def test_a_duplicate_passkey_is_reported_not_silently_dropped(self, api_client, signed_in):
        class _Result:
            credential_id = "cred"
            public_key = "pk"
            sign_count = 0
    ...
    def test_recovery_codes_that_could_not_be_written_are_never_shown(self, api_client):
    ...
    def test_a_missing_factor_is_a_404(self, api_client):
    ...
    def test_a_totp_enrolment_that_could_not_start_is_a_500(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.get_user_email", return_value=_EMAIL), \
    ...
    def test_a_pin_that_cannot_open_a_pending_login_does_not_fall_back_to_a_session(self, api_client):
    ...
    def test_the_bypass_path_fails_the_same_way(self, api_client):
        with patch(f"{_AUTH}.check_auth_init", return_value=_Allowed()), \
             patch(f"{_AUTH}.get_user_id", return_value=_UID), \
             patch(f"{_AUTH}.send_pin_email", return_value=(True, True)), \
             patch(f"{_AUTH}.has_strong_factor", return_value=True), \
    ...
    def test_a_passkey_login_that_cannot_open_a_challenge_is_a_500(self, api_client):
        with patch(f"{_M}.webauthn_relying_party"), \
    ...
    def test_a_second_factor_that_verifies_but_cannot_mint_a_session_is_a_500(self, api_client):
...
class TestFactorSummaryEndpoint:
    def test_it_reports_the_state_the_account_page_renders(self, api_client):
        from cqc_lem.utilities.auth_factors import FactorSummary
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             ...
             patch(f"{_USER}.step_up_satisfied", return_value=False):
            resp = api_client.get(f"/api/user/auth-factors?session_token={_TOKEN}")
        detail = resp.json()["detail"]
    ...
    def test_it_needs_a_session(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=None):
            resp = api_client.get("/api/user/auth-factors?session_token=nope")
        assert resp.status_code == 401
...
class TestTwoStageLoginSeams:
    ...
    def test_a_correct_pin_clears_the_limiter_before_handing_over_to_the_factor(self, api_client):
    ...
    def test_the_bypass_path_does_not_clear_it(self, api_client):
...
class TestSecondFactorBudget:
    ...
    def _pin_login(self, api_client, spent: int):
        with patch(f"{_AUTH}.check_auth_verify", return_value=_Allowed()), \
             patch(f"{_AUTH}.get_pin_lockout", return_value=None), \
             patch(f"{_AUTH}.hash_pin", return_value="h"), \
             patch(f"{_AUTH}.verify_pin_for_email", return_value=True), \
    ...
    def test_a_new_handle_inherits_what_the_account_already_spent(self, api_client):
        resp, challenge = self._pin_login(api_client, spent=3)
        assert resp.json()["detail"]["second_factor_required"] is True
        assert challenge.call_args.kwargs["initial_attempts"] == 3

    def test_a_spent_budget_refuses_to_issue_another_handle(self, api_client):
        """Five guesses per round with unlimited rounds walks a 6-digit space. The 429 is what
        makes the round cost something.
        """
        resp, challenge = self._pin_login(api_client, spent=5)
        assert resp.status_code == 429
        challenge.assert_not_called()

    def test_an_unreadable_budget_fails_closed(self, api_client):
        ...
        """
        resp, challenge = self._pin_login(api_client, spent=-1)
        assert resp.status_code == 429
        challenge.assert_not_called()

    def test_the_bypass_path_is_bounded_by_the_same_budget(self, api_client):
    ...
    def test_a_correct_code_clears_the_account_budget(self, api_client):
    ...
    def test_a_wrong_code_does_not_clear_it(self, api_client):
        with patch(f"{_AUTH}.claim_auth_challenge_attempt",
    ...
    def test_the_public_passkey_begin_is_rate_limited(self, api_client):
    ...
    def test_a_duplicate_passkey_is_not_a_cross_account_oracle(self, api_client, signed_in):
    ...
    def test_a_verified_step_up_that_does_not_stamp_is_a_500_not_a_silent_loop(self, api_client):
    ...
    def test_an_account_with_no_email_still_gets_its_own_limiter_bucket(self, api_client):
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            api_client.post("/api/user/step-up/verify",
                        json={"session_token": _TOKEN, "method": "totp", "code": "123456"})
        assert limiter.call_args.args[0] == f"user-{_UID}"

    def test_a_denied_step_up_audits_the_client_that_was_refused(self, api_client):
    ...
    def test_removing_a_factor_records_what_was_removed_and_what_is_left(self, api_client):
        from cqc_lem.utilities.auth_factors import FactorSummary
        summary = FactorSummary(

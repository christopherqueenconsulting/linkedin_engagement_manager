...
class TestBalance:
    def test_401(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=None):
            r = api_client.get("/api/video/credits", params={"session_token": "x"})
...
class TestCheckout:
    def test_200(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=1), \
            ...
            r = api_client.post("/api/video/credits/checkout",
    ...
    def test_unknown_package_400(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=1), \
             patch("cqc_lem.api.main.get_user_subscription_info", return_value={"stripe_customer_id": "cus_1"}):
            r = api_client.post("/api/video/credits/checkout",
    ...
    def test_no_customer_400(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=1), \
...
class TestUpgrade:
    def test_not_owner_403(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=1), \
             patch("cqc_lem.api.main.get_post_user_id", return_value=2):
            r = api_client.post("/api/video/upgrade", json={"session_token": "x", "post_id": 9, "tier": "premium"})
        assert r.status_code == 403

    def test_not_video_404(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=1), \
    ...
    def test_insufficient_402(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=1), \
             patch("cqc_lem.api.main.get_post_user_id", return_value=1), \
    ...
    def test_queues_200(self, api_client):
        task = MagicMock()
        with patch("cqc_lem.api.main.get_session_user_id", return_value=1), \
             patch("cqc_lem.api.main.get_post_user_id", return_value=1), \
             patch("cqc_lem.api.main.get_post_type", return_value=PostType.VIDEO), \
            ...
            r = api_client.post("/api/video/upgrade", json={"session_token": "x", "post_id": 9, "tier": "premium_top"})
...
class TestWebhookFulfillment:
    def test_video_credits_granted_on_paid(self, api_client):
    ...
    def test_video_credits_idempotent_skips_duplicate(self, api_client):
        event = {"type": "checkout.session.completed", "data": {"object": {

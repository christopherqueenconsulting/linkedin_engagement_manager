...
class TestComment:
    def test_forbidden_without_secret(self, api_client):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET):
            r = api_client.post("/api/admin/test/comment", params={"user_id": 1})
        assert r.status_code == 403

    def test_queues_task(self, api_client):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET), \
             patch("cqc_lem.api.routers.admin.automate_commenting.apply_async", return_value=_async_result()) as t:
            r = api_client.post("/api/admin/test/comment", params={"user_id": 7}, headers=_ADMIN_HEADER)
        assert r.status_code == 200
        assert r.json()["detail"]["task_id"] == "task-123"
        assert t.call_args.kwargs["kwargs"]["user_id"] == 7

    def test_missing_required_user_id_is_422(self, api_client):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET):
            r = api_client.post("/api/admin/test/comment", headers=_ADMIN_HEADER)
        assert r.status_code == 422


class TestReply:
    def test_404_when_post_user_missing(self, api_client):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET), \
             patch("cqc_lem.api.routers.admin.get_post_user_id", return_value=None):
            r = api_client.post("/api/admin/test/reply", params={"post_id": 99}, headers=_ADMIN_HEADER)
    ...
    def test_queues_task(self, api_client):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET), \
            ...
            r = api_client.post("/api/admin/test/reply", params={"post_id": 12}, headers=_ADMIN_HEADER)
        assert r.status_code == 200
...
class TestDM:
    def test_forbidden_without_secret(self, api_client):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET):
            r = api_client.post("/api/admin/test/dm", params={"user_id": 1})
        assert r.status_code == 403

    def test_queues_appreciation_dm(self, api_client):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET), \
             patch("cqc_lem.api.routers.admin.automate_appreciation_dms_for_user.apply_async",
                   return_value=_async_result()) as t:
            r = api_client.post("/api/admin/test/dm", params={"user_id": 3}, headers=_ADMIN_HEADER)
        assert r.status_code == 200
        assert t.call_args.kwargs["kwargs"]["user_id"] == 3


class TestDirectDM:
    def test_queues_direct_dm(self, api_client):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET), \
             patch("cqc_lem.api.routers.admin.send_private_dm.apply_async", return_value=_async_result()) as t:
            r = api_client.post("/api/admin/test/dm-direct",
...
class TestTaskStatus:
    def test_returns_state(self, api_client):
        res = MagicMock()
        res.state = "SUCCESS"
        ...
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET), \
            ...
            r = api_client.get("/api/admin/task-status/task-123", headers=_ADMIN_HEADER)
        assert r.status_code == 200
    ...
    def test_forbidden_without_secret(self, api_client):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET):
            r = api_client.get("/api/admin/task-status/task-123")
        assert r.status_code == 403
...
class TestConsolidateDuplicateComments:
    def test_forbidden_without_secret(self, api_client):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET):
            r = api_client.post("/api/admin/consolidate-duplicate-comments", params={"user_id": 1})
        assert r.status_code == 403

    def test_defaults_to_dry_run(self, api_client):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET), \
             patch("cqc_lem.api.routers.admin.consolidate_duplicate_comments_for_user.apply_async",
                   return_value=_async_result()) as t:
            r = api_client.post("/api/admin/consolidate-duplicate-comments",
    ...
    def test_can_request_real_delete(self, api_client):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET), \
             patch("cqc_lem.api.routers.admin.consolidate_duplicate_comments_for_user.apply_async",
                   return_value=_async_result()) as t:
            r = api_client.post("/api/admin/consolidate-duplicate-comments",
...
class TestAdminCredentialsAreNotAdvertised:
    ...
    def test_admin_schemes_absent_from_the_public_schema(self, api_client):
        schemes = api_client.get("/api/openapi.json").json().get(
    ...
    def test_admin_secret_is_still_enforced_at_runtime(self, api_client):
...
class TestAGracefulRejectionIsNotASuccess:
    ...
    def test_already_running_answers_409(self, api_client, route, task, params):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET), \
             patch("cqc_lem.api.routers.admin.get_post_user_id", return_value=5), \
             patch(f"cqc_lem.api.routers.admin.{task}.apply_async", return_value=_rejected_result()):
            r = api_client.post(route, params=params, headers=_ADMIN_HEADER)
        assert r.status_code == 409, route
        assert "already queued or in progress" in r.json()["detail"]

    @pytest.mark.parametrize("route,task,params", _QUEUE_ONCE_ROUTES)
    def test_a_real_dispatch_still_answers_200_with_its_id(self, api_client, route, task, params):
        with patch("cqc_lem.api.routers.admin.ADMIN_SECRET", _SECRET), \
             patch("cqc_lem.api.routers.admin.get_post_user_id", return_value=5), \
    ...
    @pytest.mark.parametrize("route,task,params", _QUEUE_ONCE_ROUTES)
    def test_no_route_can_ever_publish_a_null_task_id(self, api_client, route, task, params):

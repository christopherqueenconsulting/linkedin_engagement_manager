...
class TestStatusEndpoint:
    def test_requires_a_valid_session(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=None):
            assert api_client.get("/api/user/automation-status?session_token=bad").status_code == 401

    def test_healthy_account_reports_nothing_tripped(self, api_client):
    ...
    def test_a_standing_trip_is_surfaced_with_its_reason(self, api_client):
        with ExitStack() as es:
    ...
    def test_recovery_is_reported_while_the_trip_still_stands(self, api_client):
        with ExitStack() as es:
...
class TestResumeEndpoint:
    def test_requires_a_valid_session(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=None):
            response = api_client.post("/api/user/automation-resume", json={"session_token": "bad"})
        assert response.status_code == 401

    def test_clears_the_trip_and_lifts_the_tripwire_pause(self, api_client):
        with ExitStack() as es:
            _stack(es, rows=_rows(8500, 8400), trip=_TRIP,
                   pause_reason="suppression:42", pause_remaining=1000)
            ...
            detail = api_client.post("/api/user/automation-resume",
    ...
    def test_never_lifts_a_pause_the_tripwire_did_not_set(self, api_client):
    ...
    def test_never_lifts_another_users_suppression_pause(self, api_client):
    ...
    def test_is_idempotent_when_nothing_is_paused(self, api_client):
        with ExitStack() as es:
            _stack(es)
            es.enter_context(patch(f"{_RL}.clear_suppression_trip", return_value=True))
            resume = es.enter_context(patch(f"{_RL}.resume_automation", return_value=True))

...
class TestTrialExtendEndpoint:
    ...
    def test_disabled_flag_returns_404_without_touching_db(self, api_client):
        with patch(f"{_ENV}.EARLY_ADOPTER_TRIAL_ENABLED", False), \
             patch(f"{_MAIN}.extend_trial_for_user") as extend:
            resp = api_client.post(self.BASE, json={"session_token": "tok"})
    ...
    def test_invalid_session_returns_401(self, api_client):
        with patch(f"{_ENV}.EARLY_ADOPTER_TRIAL_ENABLED", True), \
             ...
             patch(f"{_MAIN}.extend_trial_for_user") as extend:
            resp = api_client.post(self.BASE, json={"session_token": "bad"})
        assert resp.status_code == 401
        extend.assert_not_called()

    def test_no_review_returns_prompt_and_grants_nothing(self, api_client):
        with patch(f"{_ENV}.EARLY_ADOPTER_TRIAL_ENABLED", True), \
    ...
    def test_review_present_grants_and_passes_feedback_id(self, api_client):
    ...
    def test_exhausted_cohorts_are_a_200_not_an_error(self, api_client):
        with patch(f"{_ENV}.EARLY_ADOPTER_TRIAL_ENABLED", True), \
             patch(f"{_MAIN}.get_session_user_id", return_value=7), \
             patch(f"{_MAIN}.get_latest_review_feedback_id", return_value=31), \
             patch(f"{_MAIN}.extend_trial_for_user",
...
class TestCheckoutSessionMirrorsGrant:
    ...
    def test_grant_holder_conversion_forwards_trial_and_coupon(self, api_client):
    ...
    def test_standard_user_conversion_is_unchanged(self, api_client):
        with patch(f"{_MAIN}.get_session_user_id", return_value=7), \
             patch(f"{_BILL}.get_user_subscription_info",
                   return_value={"stripe_customer_id": "cus_1", "stripe_subscription_id": None,
                                 "subscription_status": "trial"}), \

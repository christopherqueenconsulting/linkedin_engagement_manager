...
class TestDashboardStats:
    ...
    def test_known_user_with_no_posts_returns_zeros(self, api_client, signed_in):
    ...
    def test_start_of_month_does_not_crash(self, api_client, signed_in):
    ...
    def test_returns_counts_from_db_helper_over_all_posts(self, api_client, signed_in):
...
class TestGetActivity:
    ...
    def test_valid_user_returns_200_with_list(self, api_client, signed_in):
        log_row = {
            "id": 1,
            "action_type": "POST",
            "result": "success",
    ...
    def test_empty_log_list_returns_empty_array(self, api_client, signed_in):
        with patch(f"{_MAIN}.get_recent_logs", return_value=[]):
            resp = api_client.get(self.BASE, params={"session_token": SESSION_TOKEN})
        assert resp.status_code == 200
        assert resp.json()["detail"] == []

    def test_created_at_serialized_as_explicit_utc(self, api_client, signed_in):
        log_row = {
    ...
    def test_synthetic_feedpost_url_blanked(self, api_client, signed_in):
        rows = [
        ...
        with patch(f"{_MAIN}.get_recent_logs", return_value=rows):
            detail = api_client.get(self.BASE, params={"session_token": SESSION_TOKEN}).json()["detail"]
...
class TestUpdateUser:
    ...
    def test_no_session_returns_401(self, api_client):
        with patch(f"{_MAIN}.get_session_user_id", return_value=None):
            resp = api_client.put(self.BASE, json={"blog_url": "https://blog.example.com"})
        assert resp.status_code == 401

    def test_no_update_fields_returns_unchanged(self, api_client, signed_in):
    ...
    def test_valid_update_returns_200(self, api_client, signed_in):
    ...
    def test_update_user_returns_false_gives_404(self, api_client, signed_in):
...
class TestGetUserIdEndpoint:
    ...
    def test_returns_the_callers_own_user_id(self, api_client, signed_in):
        resp = api_client.get(self.BASE, params={"session_token": SESSION_TOKEN})
        assert resp.status_code == 200
        assert resp.json()["detail"] == SESSION_USER_ID

    def test_another_address_is_not_an_id_oracle(self, api_client, signed_in):
        resp = api_client.get(self.BASE, params={"session_token": SESSION_TOKEN,
...
class TestAuthCheckSession:
    ...
    def test_invalid_session_token_returns_401(self, api_client):
        with patch(f"{_MAIN}.get_session_user_id", return_value=None):
            resp = api_client.get(self.BASE, params={"session_token": "bad-token"})
        assert resp.status_code == 401

    def test_valid_session_returns_user_id_and_email(self, api_client, signed_in):
        with patch(f"{_MAIN}.get_session_user_id", return_value=7), \
            ...
            resp = api_client.get(self.BASE, params={"session_token": "valid-token-abc"})
        assert resp.status_code == 200
        detail = resp.json()["detail"]
    ...
    def test_returns_person_facts_for_posthog_identify(self, api_client, signed_in):
        profile = {
    ...
    def test_missing_profile_row_leaves_person_facts_null(self, api_client, signed_in):
        with patch(f"{_MAIN}.get_session_user_id", return_value=7), \
             patch(f"{_AUTH}.get_user_email", return_value="me@example.com"), \
             patch(f"{_AUTH}.get_user_analytics_profile", return_value={}), \
             patch(f"{_AUTH}.is_user_admin", return_value=False):
...
class TestGetUserSettings:
    ...
    def test_valid_session_returns_subscription_and_preferences(self, api_client, signed_in):
        sub = {
            "subscription_status": "active",
        ...
        with patch(f"{_MAIN}.get_session_user_id", return_value=5), \
            ...
            resp = api_client.get(self.BASE, params={"session_token": "valid-tok"})
        assert resp.status_code == 200
        detail = resp.json()["detail"]
    ...
    def test_none_subscription_returns_null_subscription(self, api_client, signed_in):
        with patch(f"{_MAIN}.get_session_user_id", return_value=5), \
...
class TestGetUserLinkedInProfile:
    ...
    def test_valid_session_returns_profile_url(self, api_client, signed_in):
        with patch(f"{_MAIN}.get_session_user_id", return_value=5), \
    ...
    def test_missing_profile_returns_null(self, api_client, signed_in):
        with patch(f"{_MAIN}.get_session_user_id", return_value=5), \
...
class TestCommentNotificationInbound:
    ...
    def test_comment_triggers_debounced_sweep(self, api_client):
        with patch(self._DB, return_value=7), \
            ...
            resp = api_client.post(self.BASE, data={
    ...
    def test_reaction_email_ignored(self, api_client):
        with patch(self._DB, return_value=7), \
             patch(f"{_MAIN}.sweep_reply_comments") as sweep:
            resp = api_client.post(self.BASE, data={
                "to": "reply+tok9@parse.example.com",
    ...
    def test_unknown_token_ignored(self, api_client):
    ...
    def test_no_token_ignored(self, api_client):
        with patch(f"{_MAIN}.sweep_reply_comments") as sweep:
            resp = api_client.post(self.BASE, data={"to": "x@y.com", "subject": "commented on your post"})
        assert resp.json()["detail"] == "ignored"
        sweep.apply_async.assert_not_called()

    def test_debounced_second_notification(self, api_client):
        with patch(self._DB, return_value=7), \
    ...
    def test_accepted_comment_logs_verdict(self, api_client):
        with patch(self._DB, return_value=7), \
             patch(f"{_MAIN}._reply_sweep_debounced", return_value=True), \
             patch(f"{_MAIN}.sweep_reply_comments"), \
             patch(self._TRACK) as track:
            api_client.post(self.BASE, data={
                "to": "reply+tok9@parse.example.com",
                "subject": "Chris, Jane commented on your post"})
        track.assert_called_once_with("comment_accepted", user_id=7)

    def test_unknown_token_logs_verdict(self, api_client):
        with patch(self._DB, return_value=None), \
             patch(f"{_MAIN}.sweep_reply_comments"), \
             patch(self._TRACK) as track:
            api_client.post(self.BASE, data={
    ...
    def test_linkedin_reaction_logs_verdict(self, api_client):
        with patch(self._DB, return_value=7), \
             patch(f"{_MAIN}.sweep_reply_comments"), \
             patch(self._TRACK) as track:
            api_client.post(self.BASE, data={
    ...
    def test_unrelated_mail_logs_verdict(self, api_client):
        with patch(self._DB, return_value=7), \
             patch(f"{_MAIN}.sweep_reply_comments"), \
             patch(self._TRACK) as track:
            api_client.post(self.BASE, data={
    ...
    def test_pin_endpoint_spam_logs_verdict(self, api_client):
    ...
    def test_tracking_failure_never_breaks_webhook(self, api_client):
        with patch(self._DB, return_value=7), \
             patch(f"{_MAIN}._reply_sweep_debounced", return_value=True), \
             patch(f"{_MAIN}.sweep_reply_comments") as sweep, \
    ...
    def test_gmail_forwarding_confirmation_auto_clicks(self, api_client):
    ...
    def test_gmail_confirmation_click_fails_stores_code(self, api_client):
...
class TestVerificationPinInbound:
    ...
    def test_valid_reply_stores_pin(self, api_client):
    ...
    def test_reply_but_unknown_token_ignored(self, api_client):
    ...
    def test_no_token_ignored_without_calling_store(self, api_client):
        with patch(f"{_MAIN}.submit_pin_by_token") as m:
            resp = api_client.post(self.BASE, data={"to": "someone@else.com", "text": "483920"})
        assert resp.status_code == 200
        assert resp.json()["detail"] == "ignored"
        m.assert_not_called()

    def test_no_pin_ignored(self, api_client):
        with patch(f"{_MAIN}.submit_pin_by_token") as m:
            resp = api_client.post(self.BASE, data={
    ...
    def test_endpoint_is_public_no_auth(self, api_client):
        # Under the public prefix — must not 401/403 even with no auth header.
        resp = api_client.post(self.BASE, data={"to": "x", "text": "y"})
        assert resp.status_code == 200
...
class TestGmailForwardConfirmationStorage:
    ...
    def test_confirmation_stores_status_in_redis(self, api_client):
        from unittest.mock import MagicMock
        redis = MagicMock()
        got = type("R", (), {"status_code": 200})()
        ...
        with patch("cqc_lem.utilities.db.get_user_id_by_reply_token", return_value=7), \
            ...
            resp = api_client.post("/api/linkedin/comment-notification/inbound", data={
                "to": "reply+tok9@parse.example.com",
                "from": "forwarding-noreply@google.com",
                "subject": "Gmail Forwarding Confirmation", "text": body})
        assert resp.json()["detail"] == "confirmed"
    ...
    def test_redis_write_failure_never_breaks_the_webhook(self, api_client):
        from unittest.mock import MagicMock
        redis = MagicMock()
...
class TestForwardingConfirmedByDelivery:
    ...
    def _post(self, api_client, redis, **data):
        with patch("cqc_lem.utilities.db.get_user_id_by_reply_token", return_value=7), \
             patch(f"{_MAIN}._reply_sweep_debounced", return_value=True), \
             patch(f"{_MAIN}.sweep_reply_comments"), \
             patch(self._RL, return_value=redis):
            return api_client.post(self.BASE, data={"to": "reply+tok9@parse.example.com", **data})

    def test_forwarded_comment_email_confirms_forwarding(self, api_client):
        import json
        redis = self._redis()
        resp = self._post(api_client, redis, **{
    ...
    def test_confirmed_record_never_expires(self, api_client):
        ...
        redis = self._redis()
        self._post(api_client, redis, **{"from": "messages-noreply@linkedin.com",
                                     "subject": "Jane commented on your post"})
        assert "ex" not in redis.set.call_args.kwargs

    def test_forwarded_reaction_email_confirms_but_starts_no_sweep(self, api_client):
        redis = self._redis()
        with patch("cqc_lem.utilities.db.get_user_id_by_reply_token", return_value=7), \
             patch(f"{_MAIN}.sweep_reply_comments") as sweep, \
             patch(self._RL, return_value=redis):
    ...
    def test_unrelated_mail_is_not_evidence(self, api_client):
        redis = self._redis()
        resp = self._post(api_client, redis, **{"from": "spam@example.com",
                                            "subject": "Your invoice is ready"})
        assert resp.json()["detail"] == "ignored"
        redis.set.assert_not_called()

    def test_already_confirmed_is_not_rewritten(self, api_client):
        redis = self._redis({"confirmed": True, "source": "auto_click"})
        self._post(api_client, redis, **{"from": "messages-noreply@linkedin.com",
                                     "subject": "Jane commented on your post"})
        redis.set.assert_not_called()

    def test_pending_confirmation_never_downgrades_a_confirmed_one(self, api_client):
    ...
    def test_pending_confirmation_outlives_the_code_it_carries(self, api_client):
    ...
    def test_redis_outage_on_the_delivery_path_does_not_warn(self, api_client):
...
class TestSharedInboundRouting:
    ...
    def test_pin_url_routes_reply_comment_to_sweep(self, api_client):
        with patch("cqc_lem.utilities.db.get_user_id_by_reply_token", return_value=7), \
             patch(f"{_MAIN}._reply_sweep_debounced", return_value=True), \
             patch(f"{_MAIN}.sweep_reply_comments") as sweep:
            resp = api_client.post(self.PIN, data={
                "to": "reply+tok9@parse.example.com",
                "subject": "Jane commented on your post"})
        assert resp.json()["detail"] == "accepted"
        sweep.apply_async.assert_called_once()

    def test_pin_url_routes_gmail_confirmation(self, api_client):
        got = type("R", (), {"status_code": 200})()
    ...
    def test_pin_url_still_handles_pin(self, api_client):
        with patch(f"{_MAIN}.submit_pin_by_token", return_value=1) as m:
            resp = api_client.post(self.PIN, data={
...
class TestGmailConfirmationForwardToUser:
    ...
    def test_forwards_to_user_when_click_fails(self, api_client):
        body = "verify permission https://mail.google.com/mail/vf-x\nConfirmation code: 55667788"
        with patch("cqc_lem.utilities.db.get_user_id_by_reply_token", return_value=7), \

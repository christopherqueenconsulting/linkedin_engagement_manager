...
class TestCreateTarget:
    def test_creates_pending(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
            ...
            resp = api_client.post("/api/outreach/target", json={
    ...
    def test_strips_whitespace_before_dedup_and_insert(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
    ...
    def test_blank_url_422(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
    ...
    def test_insert_failure_500(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
             patch(f"{_R}.get_outreach_target_by_url", return_value=None), \
...
class TestListTargets:
    def test_lists(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
            ...
            resp = api_client.get(
                f"/api/outreach/targets?session_token={_S}"
                "&status_filter=pending&stage_filter=comment")
        assert resp.status_code == 200
    ...
    def test_401(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=None):
            resp = api_client.get("/api/outreach/targets?session_token=bad")
        assert resp.status_code == 401
...
class TestUpdateAndCancel:
    def test_approve(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
            ...
            resp = api_client.put("/api/outreach/target",
                                  json={"session_token": _S, "target_id": 3, "action": "approve"})
        assert resp.status_code == 200
        from cqc_lem.utilities.db import OutreachStatus
        assert upd.call_args.kwargs["status"] == OutreachStatus.APPROVED

    def test_save_draft_only(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
             patch(f"{_R}.get_outreach_target_user_id", return_value=_U), \
             patch(f"{_R}.update_outreach_target", return_value=True) as upd:
            resp = api_client.put("/api/outreach/target",
                                  json={"session_token": _S, "target_id": 3, "draft_text": "hi"})
        assert resp.status_code == 200
    ...
    def test_update_404_when_not_owner(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
            ...
            resp = api_client.put("/api/outreach/target", json={"session_token": _S, "target_id": 3, "draft_text": "x"})
    ...
    def test_unknown_action_422(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
            ...
            resp = api_client.put("/api/outreach/target", json={"session_token": _S, "target_id": 3, "action": "nope"})
        assert resp.status_code == 422
    ...
    def test_no_fields_and_no_action_422(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
             patch(f"{_R}.get_outreach_target_user_id", return_value=_U), \
             patch(f"{_R}.update_outreach_target") as upd:
            resp = api_client.put("/api/outreach/target", json={"session_token": _S, "target_id": 3})
        assert resp.status_code == 422
    ...
    def test_cancel_sets_canceled(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
            ...
            resp = api_client.request("DELETE", "/api/outreach/target", json={"session_token": _S, "target_id": 3})
        assert resp.status_code == 200
        from cqc_lem.utilities.db import OutreachStatus
        upd.assert_called_once_with(3, OutreachStatus.CANCELED)

    def test_delete_404_when_not_owner(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
             patch(f"{_R}.get_outreach_target_user_id", return_value=999):
            resp = api_client.request("DELETE", "/api/outreach/target", json={"session_token": _S, "target_id": 3})
        assert resp.status_code == 404

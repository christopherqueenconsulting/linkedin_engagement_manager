...
class TestListTouches:
    def test_lists_with_filters(self, api_client):
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
            ...
            resp = api_client.get("/api/catchup/touches",
...
class TestUpdateTouch:
    def test_approve_maps_to_approved_status(self, api_client):
    ...
    def test_approve_saves_the_edited_message_in_the_same_call(self, api_client):
        from cqc_lem.utilities.db import CatchupTouchStatus
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
             patch(f"{_M}.get_catchup_touch", return_value=_touch()), \
             patch(f"{_M}.update_catchup_touch", return_value=True) as upd:
            resp = api_client.put("/api/catchup/touch", json={
    ...
    def test_blank_message_is_rejected(self, api_client, blank):
    ...
    def test_approving_a_touch_with_no_stored_message_is_rejected(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
    ...
    def test_canceling_a_touch_with_no_message_is_still_allowed(self, api_client):
        from cqc_lem.utilities.db import CatchupTouchStatus
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
             patch(f"{_M}.get_catchup_touch", return_value=_touch(message=None)), \
             patch(f"{_M}.update_catchup_touch", return_value=True) as upd:
    ...
    def test_empty_update_is_rejected(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
             patch(f"{_M}.get_catchup_touch", return_value=_touch()):
            resp = api_client.put("/api/catchup/touch", json={"session_token": _S, "touch_id": 3})
        assert resp.status_code == 422

    def test_another_users_touch_is_not_found(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
    ...
    def test_missing_touch_is_not_found(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
...
class TestCatchupPreferenceValidation:
    def test_defaults_are_the_bd_subset_and_pre_review(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
             patch(f"{_USER}.update_engagement_preferences", return_value=True) as upd:
            resp = api_client.put("/api/user/engagement-preferences", json={"session_token": _S})
        assert resp.status_code == 200
    ...
    def test_unknown_event_types_are_dropped(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
             patch(f"{_USER}.update_engagement_preferences", return_value=True) as upd:
    ...
    def test_cap_is_clamped_to_the_absolute_ceiling(self, api_client, given, expected):
    ...
    def test_message_source_is_validated(self, api_client, given, expected):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
             patch(f"{_USER}.update_engagement_preferences", return_value=True) as upd:
            resp = api_client.put("/api/user/engagement-preferences", json={
    ...
    def test_get_exposes_the_plan_allowance(self, api_client, allowance):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
            ...
            resp = api_client.get("/api/user/engagement-preferences", params={"session_token": _S})
        assert resp.status_code == 200
        assert resp.json()["detail"]["max_catchup_touches_allowed"] == allowance

    def test_bad_mode_falls_back_to_pre_review(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
             patch(f"{_USER}.update_engagement_preferences", return_value=True) as upd:
            resp = api_client.put("/api/user/engagement-preferences", json={
    ...
    def test_auto_approve_is_accepted(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_U), \
             patch(f"{_USER}.update_engagement_preferences", return_value=True) as upd:
            resp = api_client.put("/api/user/engagement-preferences", json={

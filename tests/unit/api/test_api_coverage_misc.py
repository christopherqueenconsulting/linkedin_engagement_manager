...
class TestUnauthenticatedRequests:
    ...
    def test_401_and_no_side_effect(self, api_client, case_id, method, path, body, collaborator,
                                    attr):
        with patch(f"{_M}.get_session_user_id", return_value=None):
            if collaborator:
                with patch(collaborator) as mock:
                    resp = getattr(api_client, method)(path, **({"json": body} if body else {}))
...
class TestAutomationTriggerEndpoints:
    ...
    def test_automate_reply_commenting_schedules_task(self, api_client, signed_in):
        with patch(f"{_M}.automate_reply_commenting") as task:
            resp = api_client.post(f"/api/automate_reply_commenting?post_id=9&session_token={SESSION_TOKEN}")
        assert resp.status_code == 200
        kwargs = task.apply_async.call_args[1]["kwargs"]
        assert kwargs["user_id"] == SESSION_USER_ID and kwargs["post_id"] == 9

    def test_automate_reply_commenting_403_on_someone_elses_post(self, api_client, signed_in):
    ...
    def test_automate_reply_commenting_404_on_schedule_failure(self, api_client, signed_in):
        with patch(f"{_M}.automate_reply_commenting") as task:
            task.apply_async.side_effect = RuntimeError("broker down")
            resp = api_client.post(f"/api/automate_reply_commenting?post_id=9&session_token={SESSION_TOKEN}")
        assert resp.status_code == 404

    def test_schedule_post_success(self, api_client, signed_in):
        with patch(f"{_M}.insert_post", return_value=True) as ins:
            resp = api_client.post("/api/schedule_post/", json={
    ...
    def test_schedule_post_403_writing_into_another_account(self, api_client, signed_in):
    ...
    def test_schedule_post_404_on_insert_failure(self, api_client, signed_in):
    ...
    def test_schedule_post_forwards_approved_status(self, api_client, signed_in):
    ...
    def test_schedule_post_defaults_to_pending_status(self, api_client, signed_in):
        from cqc_lem.utilities.db import PostStatus
        with patch(f"{_M}.insert_post", return_value=True) as ins:
            resp = api_client.post("/api/schedule_post/", json={
                "session_token": SESSION_TOKEN,
    ...
    def test_create_weekly_content_chains_plan_then_create(self, api_client, signed_in):
        chain_obj = MagicMock()
        with patch(f"{_M}.celery_chain", return_value=chain_obj), \
            ...
            resp = api_client.post(f"/api/create_weekly_content/?session_token={SESSION_TOKEN}")
        assert resp.status_code == 200
    ...
    def test_invite_to_company_page(self, api_client, signed_in):
        with patch(f"{_M}.automate_invites_to_company_page_for_user") as task:
            resp = api_client.post(f"/api/invite_to_li_company_page/?session_token={SESSION_TOKEN}")
        assert resp.status_code == 200
        assert task.apply_async.call_args[1]["kwargs"] == {"user_id": SESSION_USER_ID}

    def test_invite_403_for_another_account(self, api_client, signed_in):
    ...
    def test_aws_test_get_my_profile(self, api_client, signed_in):
        with patch(f"{_M}.test_get_my_profile") as task:
            resp = api_client.post(f"/api/aws_test_get_my_profile/?session_token={SESSION_TOKEN}")
        assert resp.status_code == 200
        assert task.apply_async.call_args[1]["kwargs"] == {"user_id": SESSION_USER_ID}


class TestAuthLogoutAndTokenStatus:
    def test_logout_deletes_session(self, api_client):
        with patch(f"{_AUTH}.delete_session") as ds:
            resp = api_client.post("/api/auth/logout", json={"session_token": _TOK})
        assert resp.status_code == 200
    ...
    def test_token_status_no_token_is_expired(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch("cqc_lem.utilities.db.get_user_token_info", return_value=None):
            resp = api_client.get(f"/api/user/token_status?session_token={_TOK}")
    ...
    def test_token_status_healthy_token(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
    ...
    def test_token_status_expiring_triggers_refresh(self, api_client):
    ...
    def test_token_status_no_refresh_token_skips_refresh(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch("cqc_lem.utilities.db.get_user_token_info",
...
class TestLinkedInOAuth:
    ...
    def test_auth_init_redirects_to_linkedin(self, api_client):
        ...
        with patch(f"{_M}.AuthClient", return_value=auth_client):
            resp = api_client.get("/api/auth/linkedin/?session_token=tok",
    ...
    def test_callback_invalid_state_salt_400(self, api_client):
        with patch(f"{_M}.LI_STATE_SALT", "goodsalt"):
            resp = api_client.get("/auth/linkedin/callback?code=c&state=badsalt:tok",
                              follow_redirects=False)
        assert resp.status_code == 400

    def test_callback_token_exchange_failure_redirects_with_error(self, api_client):
        auth_client = MagicMock()
    ...
    def test_callback_no_access_token_redirects_with_error(self, api_client):
        auth_client = MagicMock()
    ...
    def test_callback_userinfo_failure_redirects_with_error(self, api_client):
        auth_client = MagicMock()
        auth_client.exchange_auth_code_for_access_token.return_value = _token_response()
        restli = MagicMock()
    ...
    def test_callback_with_session_updates_logged_in_user(self, api_client):
        auth_client = MagicMock()
        auth_client.exchange_auth_code_for_access_token.return_value = _token_response()
        restli = MagicMock()
        ...
        with patch(f"{_M}.AuthClient", return_value=auth_client), \
            ...
            resp = api_client.get("/auth/linkedin/callback?code=c&state=salt:tok",
                              follow_redirects=False)
    ...
    def test_callback_without_session_or_email_errors(self, api_client):
        auth_client = MagicMock()
        auth_client.exchange_auth_code_for_access_token.return_value = _token_response()
        restli = MagicMock()
...
class TestUserSettingsAndGroups:
    def test_update_user_settings(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            resp = api_client.put("/api/user/settings", json={
    ...
    def test_update_omits_content_buffer_when_not_sent(self, api_client):
    ...
    def test_update_passes_content_buffer_through(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.update_user_preferences", return_value=True) as upd:
            resp = api_client.put("/api/user/settings", json={
    ...
    def test_out_of_range_content_buffer_is_422(self, api_client, payload):
        """Forward generation spend is capped server-side, not just in the UI."""
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.update_user_preferences", return_value=True):
            resp = api_client.put("/api/user/settings",
                              json={"session_token": _TOK, **payload})
        assert resp.status_code == 422

    def test_update_user_settings_500(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.update_user_preferences", return_value=False):
            resp = api_client.put("/api/user/settings", json={"session_token": _TOK})
    ...
    def test_get_groups(self, api_client):
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            resp = api_client.get(f"/api/user/groups?session_token={_TOK}")
        assert resp.status_code == 200
        detail = resp.json()["detail"]
    ...
    def test_get_groups_none_opted_in_for_posting(self, api_client):
    ...
    def test_put_groups(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.set_groups_enabled", return_value=True) as setg:
            resp = api_client.put("/api/user/groups", json={
                "session_token": _TOK, "groups": {"g1": False}})
        assert resp.status_code == 200
        assert setg.call_args[0] == (_UID, {"g1": False})

    def test_put_groups_accepts_per_group_post_flag(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.set_groups_enabled", return_value=True) as setg:
            resp = api_client.put("/api/user/groups", json={
                "session_token": _TOK,
    ...
    def test_put_groups_500(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.set_groups_enabled", return_value=False):
            resp = api_client.put("/api/user/groups", json={"session_token": _TOK})
        assert resp.status_code == 500

    def test_post_stats_recommendations(self, api_client):
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            resp = api_client.get(f"/api/user/post-stats?session_token={_TOK}")
        detail = resp.json()["detail"]
...
class TestEngagementAnalytics:
    ...
    def test_returns_per_post_table_and_trend(self, api_client):
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            resp = api_client.get(f"/api/user/engagement-analytics?session_token={_TOK}&days=30")
        assert resp.status_code == 200
        detail = resp.json()["detail"]
    ...
    def test_coverage_reconciles_measured_posts_with_the_account_total(self, api_client):
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            resp = api_client.get(f"/api/user/engagement-analytics?session_token={_TOK}&days=90")
        detail = resp.json()["detail"]
    ...
    def test_coverage_never_reports_a_negative_backlog(self, api_client):
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             ...
             patch(f"{_USER}.get_post_performance_rows", return_value=rows):
            resp = api_client.get(f"/api/user/engagement-analytics?session_token={_TOK}")
        assert resp.json()["detail"]["coverage"]["awaiting_capture"] == 0

    def test_flags_a_plan_over_the_promo_ceiling(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
    ...
    def test_days_clamped_to_valid_window(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.get_content_mix_counts", return_value={}), \
             patch(f"{_USER}.get_post_performance_rows", return_value=[]) as fetch:
            resp = api_client.get(f"/api/user/engagement-analytics?session_token={_TOK}&days=9999")
        assert resp.status_code == 200
        assert fetch.call_args[1]["days"] == 365          # clamped upper bound

    def test_comment_quality_block_reports_outcomes_and_hold(self, api_client):
    ...
    def test_comment_quality_is_empty_not_zero_without_readings(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.get_content_mix_counts", return_value={}), \
             patch(f"{_USER}.get_post_performance_rows", return_value=[]), \
    ...
    def test_content_quality_rollup_reports_both_periods(self, api_client):
    ...
    def test_content_quality_is_empty_not_zero_without_scores(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_USER}.get_content_mix_counts", return_value={}), \
             patch(f"{_USER}.get_post_performance_rows", return_value=[]), \
             patch("cqc_lem.utilities.linkedin.rate_limit.commenting_hold_remaining",
...
class TestPostHogStatsEndpoint:
    ...
    def test_returns_the_panel_for_the_session_user(self, api_client):
        panel = {
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            resp = api_client.get(f"/api/user/posthog-stats?session_token={_TOK}")
        assert resp.status_code == 200
...
class TestAudienceGrowth:
    ...
    def test_returns_series_deltas_and_activity(self, api_client):
        from datetime import datetime, timezone
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            resp = api_client.get(f"/api/user/audience-growth?session_token={_TOK}&days=30")
        assert resp.status_code == 200
        detail = resp.json()["detail"]
    ...
    def test_days_clamped(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            resp = api_client.get(f"/api/user/audience-growth?session_token={_TOK}&days=9999")
        assert resp.status_code == 200
...
class TestLeadMagnetAndPassword:
    ...
    def test_put_lead_magnet_rejects_bait_colliding_keyword(self, api_client, bad_kw):
    ...
    def test_put_lead_magnet_500(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
    ...
    def test_linkedin_password_saved(self, api_client, signed_in):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
    ...
    def test_linkedin_password_empty_400(self, api_client, signed_in):
        with patch(f"{_M}.get_session_user_id", return_value=_UID):
            resp = api_client.put("/api/user/linkedin-password", json={
...
class TestAvatarEndpoints:
    ...
    def test_avatar_checkout_success(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            resp = api_client.post("/api/avatar/credits/checkout", json={
    ...
    def test_avatar_checkout_no_customer_400(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
    ...
    def test_list_avatar_trainings(self, api_client):
        trainings = [{"id": 1, "training_id": "t1", "status": "succeeded"}]
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_AV}.get_avatar_trainings", return_value=trainings):
            resp = api_client.get(f"/api/avatar/trainings?session_token={_TOK}")
        assert resp.json()["detail"] == trainings

    def test_sync_training_status_terminal_state_returns_as_is(self, api_client):
        trainings = [{"id": 5, "training_id": "t5", "status": "succeeded",
                      "model_ref": "owner/m:v1", "sample_paths": [{"label": "headshot",
                                                                   "path": "images/a/h.webp"}]}]
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_AV}.get_avatar_trainings", return_value=trainings):
            resp = api_client.get(f"/api/avatar/training/5/status?session_token={_TOK}")
        assert resp.json()["detail"]["status"] == "succeeded"

    def test_sync_training_status_queues_samples_on_first_success(self, api_client):
    ...
    def test_sync_training_status_second_poll_mid_render_queues_nothing(self, api_client):
    ...
    def test_sync_training_status_broker_failure_releases_the_claim(self, api_client):
        trainings = [{"id": 5, "training_id": "t5", "status": "succeeded",
                      "model_ref": "owner/m:v1", "sample_paths": []}]
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_AV}.get_avatar_trainings", return_value=trainings), \
    ...
    def test_sync_training_status_does_not_requeue_when_samples_exist(self, api_client):
        trainings = [{"id": 5, "training_id": "t5", "status": "succeeded",
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_AV}.get_avatar_trainings", return_value=trainings), \
             patch("cqc_lem.app.run_avatar.render_avatar_samples_task.apply_async") as queued:
            api_client.get(f"/api/avatar/training/5/status?session_token={_TOK}")
        queued.assert_not_called()

    def test_sync_training_status_polls_replicate_for_running(self, api_client):
    ...
    def test_sync_training_status_404_unknown_id(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_AV}.get_avatar_trainings", return_value=[]):
            resp = api_client.get(f"/api/avatar/training/99/status?session_token={_TOK}")
        assert resp.status_code == 404

    def test_activate_avatar_success(self, api_client):
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            resp = api_client.put("/api/avatar/training/5/activate",
                              json={"session_token": _TOK})
        assert resp.status_code == 200
        act.assert_called_once_with(_UID, 5)

    def test_activate_avatar_rejects_unapproved(self, api_client):
    ...
    def test_activate_avatar_rejects_unfinished_training(self, api_client):
    ...
    def test_activate_avatar_404(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch(f"{_AV}.get_avatar_training", return_value=None):
            resp = api_client.put("/api/avatar/training/99/activate",
                              json={"session_token": _TOK})
        assert resp.status_code == 404
...
class TestAdminEndpoints:
    ...
    def test_forbidden_without_secret(self, api_client, case_id, path, body):
        with patch(f"{_ADMIN}.ADMIN_SECRET", "sekret"):
            resp = api_client.post(path, headers={}, **({"json": body} if body else {}))
        assert resp.status_code == 403

    def test_automation_pause(self, api_client):
        with patch(f"{_ADMIN}.ADMIN_SECRET", "sekret"), \
             patch("cqc_lem.utilities.linkedin.rate_limit.pause_automation", return_value=True) as pause:
            resp = api_client.post("/api/admin/automation-pause?hours=6", headers=self._HDR)
        assert resp.status_code == 200
        assert resp.json()["detail"] == {"paused": True, "seconds": 6 * 3600}
        assert pause.call_args[0][0] == 6 * 3600

    def test_automation_resume(self, api_client):
        with patch(f"{_ADMIN}.ADMIN_SECRET", "sekret"), \
             patch("cqc_lem.utilities.linkedin.rate_limit.resume_automation", return_value=True):
            resp = api_client.post("/api/admin/automation-resume", headers=self._HDR)
        assert resp.status_code == 200
        assert resp.json()["detail"] == {"resumed": True}

    def test_automation_status(self, api_client):
        with patch(f"{_ADMIN}.ADMIN_SECRET", "sekret"), \
    ...
    def test_regenerate_carousel_success(self, api_client):
        ...
        with patch(f"{_ADMIN}.ADMIN_SECRET", "sekret"), \
            ...
            resp = api_client.post("/api/admin/regenerate-carousel", headers=self._HDR,
    ...
    @pytest.mark.parametrize("path", ["/api/admin/regenerate-carousel",
                             ...
                             ids=["carousel", "video"])
    def test_regenerate_404_for_the_wrong_post_type(self, api_client, path):
        from cqc_lem.utilities.db import PostType
        with patch(f"{_ADMIN}.ADMIN_SECRET", "sekret"), \
             patch(f"{_ADMIN}.get_post_type", return_value=PostType.TEXT):
            resp = api_client.post(path, headers=self._HDR, json={"post_id": 9, "user_id": 1})
        assert resp.status_code == 404

    def test_regenerate_carousel_500_on_failure(self, api_client):
        from cqc_lem.utilities.db import PostType
        with patch(f"{_ADMIN}.ADMIN_SECRET", "sekret"), \
             patch(f"{_ADMIN}.get_post_type", return_value=PostType.CAROUSEL), \
    ...
    def test_regenerate_video_success(self, api_client):
        from cqc_lem.utilities.db import PostType
        with patch(f"{_ADMIN}.ADMIN_SECRET", "sekret"), \
            ...
            resp = api_client.post("/api/admin/regenerate-video", headers=self._HDR,
                               json={"post_id": 9, "user_id": 1})
        assert resp.status_code == 200
        assert resp.json()["detail"]["video_url"].endswith("v.mp4")

    def test_regenerate_video_500_when_no_asset(self, api_client):
        from cqc_lem.utilities.db import PostType
        with patch(f"{_ADMIN}.ADMIN_SECRET", "sekret"), \
             patch(f"{_ADMIN}.get_post_type", return_value=PostType.VIDEO), \
             patch("cqc_lem.app.run_content_plan.regenerate_video_for_post",
...
class TestCarouselTemplatesAndPreview:
    def test_list_templates_returns_catalog(self, api_client):
        resp = api_client.get("/api/carousel-templates")
        assert resp.status_code == 200
        templates = resp.json()["detail"]["templates"]
        assert templates and {"key", "label", "description"} <= set(templates[0])

    def test_generate_carousel_success(self, api_client, tmp_path):
        ...
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
            ...
            resp = api_client.post("/api/generate-carousel", json={
    ...
    def test_generate_carousel_500_on_generation_error(self, api_client):
        with patch(f"{_M}.get_session_user_id", return_value=_UID), \
             patch("cqc_lem.utilities.ai.ai_helper.generate_carousel_content",
...
class TestAssetsCompatRedirect:
    def test_redirects_to_api_assets(self, api_client):
        resp = api_client.get("/assets?file_name=videos/x.mp4", follow_redirects=False)
        assert resp.status_code == 301
        assert resp.headers["location"] == "/api/assets?file_name=videos/x.mp4"

    def test_404_without_file_name(self, api_client):
        resp = api_client.get("/assets", follow_redirects=False)
        assert resp.status_code == 404
...
class TestStripeCreditWebhooks:
    ...
    def _post(self, api_client, event, extra_patches=()):
        with patch("cqc_lem.utilities.stripe_util.validate_webhook", return_value=event):
            for p in extra_patches:
                p.start()
            try:
                return api_client.post(self.BASE, content=b"{}", headers=self.HDRS)
            finally:
                for p in extra_patches:
                    p.stop()

    def test_invoice_payment_succeeded_refetches_subscription(self, api_client):
        ...
        with patch("cqc_lem.utilities.stripe_util.validate_webhook", return_value=event), \
            ...
            resp = api_client.post(self.BASE, content=b"{}", headers=self.HDRS)
        assert resp.status_code == 200
        fetch.assert_called_once_with("sub_1")
        args = upd.call_args[0]
        assert args[0] == "cus_1" and args[1] == "active" and args[2] == "pro"

    def test_avatar_credit_checkout_grants_credits(self, api_client):
    ...
    def test_avatar_credit_checkout_idempotent_on_retry(self, api_client):
        event = _event("checkout.session.completed", {
            "id": "cs_1", "customer": "cus_1", "payment_status": "paid",
    ...
    def test_avatar_credit_checkout_unpaid_skips_grant(self, api_client):
        event = _event("checkout.session.completed", {
    ...
    def test_video_credit_checkout_grants_credits(self, api_client):
        event = _event("checkout.session.completed", {
    ...
    def test_full_refund_deducts_avatar_credits(self, api_client):
    ...
    def test_partial_refund_makes_no_adjustment(self, api_client):
        event = _event("charge.refunded", {
            "payment_intent": "pi_1", "customer": "cus_1",
    ...
    def test_refund_of_non_credit_charge_ignored(self, api_client):
        event = _event("charge.refunded", {
            "payment_intent": "pi_1", "customer": "cus_1",
            "amount": 1000, "amount_refunded": 1000})

...
class TestMessageThreadProbe:
    ...
    def test_the_search_route_is_skipped_and_grades_unknown_not_broken(self, monkeypatch):
        """The production ladder's last resort types a name and presses Enter.

        The #1301 guard refuses that, and the ladder's own broad ``except Exception`` would swallow
        the refusal and report an ordinary failed walk (the #1857 defect). So the probe SKIPS that
        route: a walk that then opens nothing must grade UNKNOWN naming the guard, not read as a
        broken route.
        """

...
def _upload(api_client, data=None, post_id=_POST, name="shot.png"):
    payload = {"session_token": _SESSION}
    if post_id is not None:
        payload["post_id"] = post_id
    return api_client.post("/api/user/post/image", data=payload,
                       files={"file": (name, data or _png(), "image/png")})


def _generate(api_client, **body):
    return api_client.post("/api/user/post/image/generate",
...
class TestUploadLifecycle:
    def test_upload_attaches_the_image_to_the_row_and_writes_the_file(self, api_client, store):
    ...
    def test_re_uploading_replaces_the_file_and_deletes_the_old_one(self, api_client, store):
        first = _upload(api_client).json()["detail"]["image_url"]
        first_path = _abs(store, first)
        second = _upload(api_client, _png(1200, 1200)).json()["detail"]["image_url"]
        assert first != second
        assert not os.path.exists(first_path)
        assert os.path.isfile(_abs(store, second))

    def test_a_format_linkedin_will_not_take_is_a_400_and_the_row_keeps_nothing(self, api_client, store):
        resp = _upload(api_client, _png(fmt="GIF"), name="shot.gif")
        assert resp.status_code == 400
        assert "PNG or JPG" in resp.json()["detail"]
        assert store.row["image_url"] is None

    def test_a_thumbnail_sized_upload_is_a_400(self, api_client, store):
        assert _upload(api_client, _png(100, 100)).status_code == 400
        assert store.row["image_url"] is None

    def test_remove_clears_the_row_and_deletes_the_file(self, api_client, store):
        url = _upload(api_client).json()["detail"]["image_url"]
        path = _abs(store, url)
        resp = api_client.post("/api/user/post/image/remove",
...
class TestComposeTimePreview:
    def test_an_upload_with_no_post_id_touches_no_row(self, api_client, store):
        resp = _upload(api_client, post_id=None)
        assert resp.status_code == 200
        url = resp.json()["detail"]["image_url"]
    ...
    def test_scheduling_carries_this_accounts_preview_onto_the_new_post(self, api_client, store):
        url = _upload(api_client, post_id=None).json()["detail"]["image_url"]
    ...
    def test_a_url_we_never_issued_is_dropped_rather_than_stored(self, api_client, store):
...
@pytest.mark.integration
class TestGenerate:
    def test_generation_attaches_the_render_and_prefers_the_text_on_screen(self, api_client, store):
        with patch("cqc_lem.api.routers.user.generate_image_for_post",
                   return_value=("https://api.test/api/assets?file_name=images/posts/42/g.png",
                                 None)) as gen:
            resp = _generate(api_client, post_id=_POST, content="Unsaved edit")
        assert resp.status_code == 200
        assert gen.call_args[0][1] == "Unsaved edit"
        assert store.row["image_url"].endswith("g.png")

    def test_generation_falls_back_to_the_stored_draft(self, api_client, store):
        with patch("cqc_lem.api.routers.user.generate_image_for_post",
                   return_value=("https://api.test/api/assets?file_name=images/posts/42/g.png",
                                 None)) as gen:
            assert _generate(api_client, post_id=_POST).status_code == 200
        assert gen.call_args[0][1] == "Stored draft text"

    def test_a_compose_time_generation_needs_no_post(self, api_client, store):
        ...
        with patch("cqc_lem.api.routers.user.generate_image_for_post", return_value=(preview, None)):
            resp = _generate(api_client, content="Fresh copy")
        assert resp.status_code == 200
        assert resp.json()["detail"] == {"post_id": None, "image_url": preview}
        assert store.row["image_url"] is None

    def test_a_draft_longer_than_a_linkedin_post_still_generates(self, api_client, store):
        ...
        with patch("cqc_lem.api.routers.user.generate_image_for_post",
                   return_value=("https://api.test/api/assets?file_name=images/posts/42/g.png",
                                 None)) as gen:
            resp = _generate(api_client, post_id=_POST, content="x" * 4000)
        assert resp.status_code == 200
        assert len(gen.call_args[0][1]) == 4000

    def test_nothing_written_is_a_400_before_any_spend(self, api_client, store):
    ...
    def test_the_hourly_cap_refuses_before_the_render(self, api_client, store):
        with patch("cqc_lem.api.routers.user.claim_manual_generation", return_value=False), \
            ...
            resp = _generate(api_client, post_id=_POST, content="More please")
...
class TestOwnershipAndState:
    def test_another_users_post_is_a_404_on_every_image_route(self, api_client, tmp_path):
        ...
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             ...
             patch("cqc_lem.api.routers.user.generate_image_for_post") as gen:
            assert _upload(api_client).status_code == 404
            assert _generate(api_client, post_id=_POST, content="x").status_code == 404
            assert api_client.post("/api/user/post/image/remove", json={
                "session_token": _SESSION, "post_id": _POST}).status_code == 404
        write.assert_not_called()
        gen.assert_not_called()
        assert not os.path.exists(os.path.join(str(tmp_path), "images"))

    def test_a_published_post_is_a_409(self, api_client, store):
        store.row["status"] = "posted"
        with patch("cqc_lem.api.routers.user.generate_image_for_post") as gen:
            assert _upload(api_client).status_code == 409
            assert _generate(api_client, post_id=_POST, content="x").status_code == 409
            assert api_client.post("/api/user/post/image/remove", json={
                "session_token": _SESSION, "post_id": _POST}).status_code == 409
        gen.assert_not_called()
        assert store.row["image_url"] is None

    def test_no_session_is_a_401(self, api_client, tmp_path):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=None), \
             patch("cqc_lem.api.routers.user.record_auth_event", return_value=True), \
             patch("cqc_lem.utilities.post_image.assets_dir", str(tmp_path)):
            assert _upload(api_client, post_id=None).status_code == 401
            assert _generate(api_client, content="x").status_code == 401
        assert not os.path.exists(os.path.join(str(tmp_path), "images"))

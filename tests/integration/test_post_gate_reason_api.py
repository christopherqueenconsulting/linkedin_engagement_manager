...
class TestRescoreEndpoint:
    def test_promotes_a_post_that_now_passes(self, api_client):
        ...
        with patch(f"{_API}.get_session_user_id", return_value=1), \
            ...
            r = api_client.post("/api/user/post/rescore",
                               json={"session_token": "t", "post_id": 42})
        assert r.status_code == 200
        assert r.json()["detail"] == result
        rescore.assert_called_once_with(42)

    def test_returns_the_findings_when_it_still_fails(self, api_client):
    ...
    def test_404_for_someone_elses_post(self, api_client):
        with patch(f"{_API}.get_session_user_id", return_value=1), \
    ...
    def test_409_outside_the_review_states(self, status, api_client):
        with patch(f"{_API}.get_session_user_id", return_value=1), \
             patch(f"{_USER}.get_post_user_id", return_value=1), \
    ...
    def test_500_when_scoring_blows_up(self, api_client):
        with patch(f"{_API}.get_session_user_id", return_value=1), \
             patch(f"{_USER}.get_post_user_id", return_value=1), \
             patch(f"{_USER}.get_post_status", return_value="pending"), \
...
class TestThresholdPreferences:
    def test_thresholds_round_trip_through_the_prefs_endpoints(self, api_client):
        with patch(f"{_API}.get_session_user_id", return_value=1), \
    ...
    def test_out_of_range_thresholds_are_clamped_at_the_boundary(self, api_client):
        with patch(f"{_API}.get_session_user_id", return_value=1), \
             patch(f"{_USER}.update_engagement_preferences", return_value=True) as save:
            api_client.put("/api/user/engagement-preferences",
    ...
    def test_get_exposes_the_defaults_the_ui_shows_as_placeholders(self, monkeypatch, api_client):
        ...
        with patch(f"{_API}.get_session_user_id", return_value=1), \
            ...
            r = api_client.get("/api/user/engagement-preferences?session_token=t")

...
def _upload(api_client, data=None):
    return api_client.post("/api/user/newsletter-draft/cover",
                       data={"session_token": _SESSION, "edition_id": _EDITION},
                       files={"file": ("cover.png", data or _png(), "image/png")})


def _queue(api_client):
    return api_client.get(f"/api/user/newsletter-draft?session_token={_SESSION}").json()["detail"]


@pytest.mark.integration
class TestUploadLifecycle:
    def test_upload_lands_approved_and_shows_up_on_the_queue(self, api_client, store):
        assert _upload(api_client).status_code == 200
        edition = _queue(api_client)["editions"][0]
    ...
    def test_the_uploaded_file_is_on_disk_under_the_users_own_folder(self, api_client, store):
        assert _upload(api_client).status_code == 200
    ...
    def test_re_uploading_replaces_the_file_and_deletes_the_old_one(self, api_client, store):
        ...
        assert _upload(api_client, _png(1600, 900)).status_code == 200
        second = os.path.join(store.assets_dir, store.row["cover_image_path"])
        assert first != second
        assert not os.path.exists(first)
        assert os.path.isfile(second)

    def test_a_portrait_image_is_rejected_and_the_edition_keeps_no_cover(self, api_client, store):
        resp = _upload(api_client, _png(700, 1400))
        assert resp.status_code == 400
        assert store.row["cover_image_path"] is None
        assert _queue(api_client)["editions"][0]["cover_image_url"] is None

    def test_remove_takes_the_row_and_the_file(self, api_client, store):
        assert _upload(api_client).status_code == 200
...
class TestGeneratedCoverGate:
    def test_a_generated_cover_is_pending_until_the_author_approves_it(self, api_client, store):
    ...
    def test_approving_an_edition_with_no_cover_is_a_404(self, api_client, store):
        resp = api_client.post("/api/user/newsletter-draft/cover/decision", json={
            "session_token": _SESSION, "edition_id": _EDITION, "action": "approve"})
...
class TestCoverOwnership:
    def test_another_users_edition_is_a_404_on_every_cover_route(self, api_client, tmp_path):
        ...
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
            ...
            assert api_client.post("/api/user/newsletter-draft/cover/generate", json={
                "session_token": _SESSION, "edition_id": _EDITION}).status_code == 404
            assert api_client.post("/api/user/newsletter-draft/cover/decision", json={
                "session_token": _SESSION, "edition_id": _EDITION,

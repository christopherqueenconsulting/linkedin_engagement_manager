...
class TestTheGuardIsReachableAtRuntime:
    ...
    def test_the_refusal_leaves_the_row_unapproved(self, client, agent_token, pending_request):
        """A 403 that still wrote the row would be theatre."""
        client.put("/api/connection_request",
                   json={"session_token": agent_token, "request_id": pending_request,
                         "action": "approve"})
    ...
    def test_an_agent_may_still_save_a_draft(self, client, agent_token, pending_request):
...
class TestApprovalAtCreateTime:
    ...
    def test_an_agent_cannot_create_an_already_approved_dm(self, client, agent_token):
        when = (datetime.now(timezone.utc) + timedelta(hours=2)).strftime("%Y-%m-%d %H:%M:%S")
        r = client.post("/api/schedule_dm",
    ...
    def test_an_agent_may_create_a_pending_dm(self, client, agent_token):
        when = (datetime.now(timezone.utc) + timedelta(hours=2)).strftime("%Y-%m-%d %H:%M:%S")
        r = client.post("/api/schedule_dm",
                        json={"session_token": agent_token,
                              "recipient_profile_url": "https://www.linkedin.com/in/someone-1026/",
...
class TestTheRestOfTheNarrowing:
    def test_an_agent_cannot_reconfigure_the_account(self, client, agent_token):
        """This write sets the approval modes and the per-day caps — it re-opens everything else."""
        r = client.put("/api/user/engagement-preferences",
                       json={"session_token": agent_token, "connection_request_mode": "auto_approve",
                             "max_dms_per_day": 500})
        assert r.status_code == 403, r.text
        assert r.json()["detail"]["code"] == "agent_may_not_configure"

    def test_an_agent_may_still_read_its_preferences(self, client, agent_token):
        r = client.get(f"/api/user/engagement-preferences?session_token={agent_token}")
        assert r.status_code == 200, r.text
    ...
    def test_a_path_off_the_surface_is_refused_before_any_handler(self, client, agent_token,
                                                                  method, path):
...
class TestTheGrantedTtlSurvivesBeingUsed:
    ...
    def test_a_browser_session_still_slides(self, client, human_token):
        ...
        client.get(f"/api/user/engagement-preferences?session_token={human_token}")

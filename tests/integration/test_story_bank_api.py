...
class TestGetStoryBank:
    def test_returns_entries_kinds_and_the_seeding_target(self, api_client):
        ...
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
            ...
            resp = api_client.get(f"/api/user/story-bank?session_token={_SESSION}")
    ...
    def test_401(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=None):
            resp = api_client.get("/api/user/story-bank?session_token=bad")
        assert resp.status_code == 401


@pytest.mark.integration
class TestUpdateStoryBank:
    def test_seeds_new_entries(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.upsert_story_bank_entries", return_value=True) as upd:
            resp = api_client.put("/api/user/story-bank", json={
    ...
    def test_unknown_kind_falls_back_to_anecdote(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.upsert_story_bank_entries", return_value=True) as upd:
            resp = api_client.put("/api/user/story-bank", json={
                "session_token": _SESSION,
    ...
    def test_editing_an_existing_entry_keeps_its_id(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.upsert_story_bank_entries", return_value=True) as upd:
            api_client.put("/api/user/story-bank", json={
                "session_token": _SESSION,
                "entries": [{"id": 9, "body": "edited", "active": False}]})
        saved = upd.call_args[0][1][0]
        assert saved["id"] == 9 and saved["active"] is False

    def test_missing_body_is_422(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER):
            resp = api_client.put("/api/user/story-bank", json={
                "session_token": _SESSION, "entries": [{"title": "no body"}]})
        assert resp.status_code == 422

    def test_over_long_body_is_422(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER):
            resp = api_client.put("/api/user/story-bank", json={
    ...
    def test_500_on_failure(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
            ...
            resp = api_client.put("/api/user/story-bank",
...
class TestDeleteStoryBankEntry:
    def test_deletes_scoped_to_the_session_user(self, api_client):
        with patch("cqc_lem.api.main.get_session_user_id", return_value=_USER), \
             patch("cqc_lem.api.routers.user.delete_story_bank_entry", return_value=True) as dele:
            resp = api_client.request("DELETE", "/api/user/story-bank",

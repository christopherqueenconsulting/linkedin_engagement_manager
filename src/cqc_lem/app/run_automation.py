...
DMs and their follow-up ladders, appreciation, the profile-viewer walk, the outreach funnel and the
catch-up lane dispatch from here; the reusable Selenium mechanics they share live in
`utilities/linkedin/*`. The clusters that have left (#1154) are re-exported at the bottom of the
import block and live in `app/engagement/`: feed + groups + roster in `feed.py`, the connect rail in
`invites.py`, the newsletter rail in `newsletter.py`, and publishing plus the post-publish sweeps in
`posting.py`. The per-beat posture — what each one guarantees and why — is
...
# Posting and the post-publish sweeps moved to `app.engagement.posting` (#1154). Nothing in THIS
# file calls any of them — the ten are re-exported because `run_scheduler` imports six by name
# (`post_to_linkedin`, the three sweeps, `update_stale_profile`, plus `auto_scrape_post_stats` and
# `capture_follower_stats` lazily), `api/main.py` imports `automate_reply_commenting` and
# `sweep_reply_comments`, `api/routers/admin.py` the first of those and `api/routers/user.py`
# `update_stale_profile`. `process_comment_followups_for_url` and `reconcile_recent_comment_urns`
# ride along so the cluster keeps ONE import path rather than two that differ for no reason a reader
# can see. Each task pins `name='cqc_lem.app.run_automation.<fn>'`, so the module path is all that
# changed about them.
#
# The ~60 PRIVATE helpers that moved with them — the reply rail
# (`_reply_to_comments_on_open_post`), the follow-up sweep, the comment-sort control walk, the
# outcome reader and the post-stats parsers — are deliberately NOT re-exported. A re-export would
# keep a stale `patch("...run_automation._find_our_comment")` resolving while the sweep reads its
# own module's binding: the patch would bind nothing and the test would pass having tested nothing.
# Leaving them absent makes that an AttributeError at patch time.
from cqc_lem.app.engagement.posting import (  # noqa: E402  (after the local import block on purpose)
...
__all__ = [
    ...
    "auto_publish_newsletter_edition",
    "track_newsletter_subscribers",
    # The posting + post-publish sweep tasks re-exported above.
    "auto_scrape_post_stats",
    "automate_reply_commenting",
    "capture_follower_stats",
    "post_to_linkedin",

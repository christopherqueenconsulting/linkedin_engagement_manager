...
- **Message-thread resolution ladder** (`utilities/linkedin/message_thread.py`, issue #731): the ONE way LEM opens (and reads) a 1:1 thread. `open_message_thread` walks SIX routes in order and **a route only counts when the thread is provably open**. Class names never keyed on — every locator is href / aria-label / TEXT. Verdict is **three-valued**: `ThreadState.REPLIED` / `NOT_REPLIED` / `UNKNOWN` — UNKNOWN makes the caller SKIP (a missed follow-up is recoverable, a follow-up to a reply is not). Self-name is a **required setting** (`users.linkedin_display_name`), not a scrape. Full posture: `docs/engagement-automation.md`.
- **Owned-asset CTA loop** (`resolve_artifact_delivery` in `content_alignment.py` + `_queue_artifact_delivery`, issue #624): the ONE map from a CTA to its asset, and it names the CHANNEL — **lead magnet** is the comment-keyword mechanic whose payload is a DM; **newsletter** is a subscribe LINK. Keyword delivery is **approval-gated** (`pending` `scheduled_dms` row, `source='artifact'`). Full posture: `docs/engagement-automation.md`.
- **Human pacing** (`utilities/human_pacing.py`, issue #626): the ONE place cadence is decided — read-time delay, dispatch jitter, and a per-day-cap random draw, all seeded on (user, action, date) and persisted in Redis so a retry never re-rolls. Fails open (no Redis / `HUMAN_PACING_ENABLED=false`). Pacing only slows us down; the 429 breaker in `rate_limit.py` is the separate, harder gate. Full posture: `docs/engagement-automation.md`.
- **Comment outcome tracking** (`sweep_comment_outcomes` + `utilities/comment_outcomes.py`, issue #628): read-only T+24h sweep writes ONE `comment_outcomes` row per comment — author/thread replies, likes, and a **three-valued** `visible_most_relevant` (1 relevant / 0 demoted / NULL unreadable, NULL excluded from the denominator). Demotion rate over threshold **holds that user's feed commenting** and escalates CRITICAL. Full posture: `docs/engagement-automation.md`.
- **Suppression tripwire** (`auto_suppression_tripwire` + `utilities/suppression.py`, issue #629): 2026 LinkedIn penalties are SILENT (reach step-collapses, no notification). A daily beat compares impressions-per-post against the user's OWN trailing 14-day median; a sustained drop `pause_automation()`s **engagement only** (posting is never gated) and escalates CRITICAL. Cold start / thin baseline = `unknown`, never actioned. Recovery is human (`POST /user/automation-resume`). Full posture: `docs/engagement-automation.md`.
- **Weekly group post** (`auto_draft_group_post` → `auto_post_to_group`, issue #932): TWO beats with a review window between them. `auto_draft_group_post` (Sun) is the ONE place a group post's text is written — no browser, cached profile — and `auto_post_to_group` (Tue) publishes that draft and generates NOTHING, so **a run with no READY draft publishes nothing**. Silence ships it (resting status is `ready`, not pending approval), so the cadence is unchanged; the SPA (`GroupsCard`, `GET`/`PUT /user/group-post-draft`) is where a user rewrites or skips it. ONE open draft per user — it may hold their edits, so it is carried forward, never replaced. Full posture: `docs/engagement-automation.md`.
- **Company-page invitations** (`utilities/linkedin/company_page_inviter.py`, issue #732): a paced DAILY drip bounded by the SMALLEST of three ceilings — per-day cap, credit spread (`credits_remaining / days_left_in_month`), and live credit count. `plan_daily_invites` decides all of that BEFORE a Chrome session opens — most days the allowance is zero. Full posture: `docs/engagement-automation.md`.
...
- **LEM identity + sessions** (#745 phase 2b): `users.public_uid` is the identity, email is a movable ATTRIBUTE (`change_user_email` + `user_email_history`, PIN to the NEW address, every other session revoked). `sessions.session_token` stores `SHA-256(token)` — UNKEYED on purpose, so a rotated `LEM_SECRET_KEY` can never log everyone out — and the token itself rides in an **httpOnly** cookie. `api/main.get_session_user_id()` is the ONE resolver: explicit token that RESOLVES wins, else the request's cookie off a ContextVar; the SPA sends the non-secret sentinel `'cookie'` so ~150 call sites keep their shape while holding no secret. **Since #914 EVERY `/api` route resolves its caller through it** — `require_session_user_id()` is that resolver plus a 401, and an `email`/`user_id`/`post_id` in the request is a TARGET to authorise (`_reject_foreign_email` / `_reject_foreign_user_id` / `_require_own_posts` → 403), never the actor. The old shape authenticated on the parameter behind only the shared bearer token the SPA ships in its build, so anyone who loaded the page could name an account and act on it. `db.user_owns_posts` FAILS CLOSED and judges a batch whole; a route that imports `db.get_session_user_id` around the wrapper is a bug. Per-device rows are listable/revocable (`GET /user/security`, `POST /user/sessions/revoke` — scoped by `user_id`, so a foreign id is a 404). Auth limiting is TWO layers: Redis windows in `utilities/auth_rate_limit.py` (**fails open**) and the durable PIN lockout in `verify_pin_for_email` (the hard gate). Everything lands in `auth_audit_log` (`user_id` nullable — a failed login has no account). Full: `docs/identity-and-sessions.md`.
- **Strong auth + step-up** (#745 phase 2c): `utilities/auth_factors.py` is the ONE place a user's factor state is decided (`webauthn_util.py` holds the ceremonies). Once an account enrols a passkey or TOTP, the email PIN is a **bootstrap** — it mints no session, it mints a pending handle (the no-mail-provider bypass takes the same branch). A **passkey** login is the only phishing-resistant path and the only one that arrives already stepped up; a **recovery code** signs you in but never steps up. `sessions.last_verified_at` gates every credential-touching write (`li_at`, password, email change, session revoke, factor removal, recovery regeneration) — refusal is **403 `step_up_required`**, never 401 (the SPA reads 401 as a dead session). **The FIRST factor is free, every one after it is gated, removing one always is** (`enrollment_allowed` — enrolling STAMPS the session, so an ungated one is a free step-up); an account with NO factor passes (opt-in rollout), and a `scope='recovery'` session may enrol without proving anything but is never stamped for it. A pending second-factor handle survives a wrong code and is burned by the `SECOND_FACTOR_MAX_ATTEMPTS`-th (durable, in `auth_challenges.attempts` — the Redis limiter fails open), and that budget is counted **per account** over `SECOND_FACTOR_ATTEMPT_WINDOW_MINUTES` and carried into the next handle, so starting the sign-in over never buys fresh guesses: **401 = wrong code, retry; 400 = handle gone, start over; 429 = budget spent**. The extension can't run WebAuthn, so its step-up happens once in the SPA at `/user/extension-token`, which mints a `scope='extension'` session. `STRONG_AUTH_ENABLED=false` reverts to 2b without deleting a factor. Full: `docs/strong-authentication.md`.
- **Session scopes are SURFACES** (#905, 2c.1): `api/main.get_session_user_id` is the ONE place a scope is enforced — a narrowing remembered at ~150 call sites is a narrowing that leaks. `extension` reaches the ONE path the extension calls (anything else is 403 + an audited `session_scope_denied`); `enroll` — a PIN login past `REQUIRE_STRONG_FACTOR_AFTER` on a factor-less account — reaches only the enrolment surface, and enrolling promotes it to `full`. **A hold is never a lockout:** the PIN still signs you in, and every read goes through `enrollment_hold_active()`, so clearing/moving the date releases the sessions it already held. Empty date (the default) = 2c behaviour exactly.

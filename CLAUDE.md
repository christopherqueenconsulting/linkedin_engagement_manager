...
| **Admin user management** (#1450) | `/api/admin/users*` in `api/routers/admin.py`; `AdminUsersPage.tsx` | Read-heavy, with ONE write: grant/revoke admin, step-up gated and audited to `auth_audit_log` against the TARGET. "Admin" is always `is_user_admin`'s predicate — the column **OR** `ADMIN_USER_EMAILS` — so the last-admin guard counts the EFFECTIVE admins and an **unreadable count is 503, never 0**; self-revoke and an allowlist admin are refused (409). Nothing else is editable: subscription is Stripe's, preferences are their owner's, and there is no per-user disable to show | `docs/admin-user-management.md` |

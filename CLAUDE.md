...
- **Roster targets LEM can't comment on** (`comment_on_roster_posts` + `auto_follow_roster_target`, issue #962): an author who restricts commenting renders NO comment affordance, so the roster skipped them **invisibly**. A visit that finds posts but zero commentable cards is the restricted-comments signature and records a blocked visit (`record_target_comment_blocked`); "no posts at all" records nothing, and neither does a walk the run's deadline cut short. The per-visit skip is DEBUG (working behaviour on their side) — only the streak CROSSING the badge threshold logs INFO, once. A landed comment clears the streak in `record_target_engagement`'s own statement. Blocked visits are written AFTER the walk (`_record_blocked_visits`) so ONE run-level check can tell a restricted roster from `_card_for_textbox` drift — every target on a 3+ roster reading blocked is drift, warned and never persisted. **Auto-follow is opt-in and OFF by default** (`roster_auto_follow`), runs only on the page the roster pass already opened and only AFTER its comment walk, and draws its own paced budget (`ACTION_FOLLOW` / `max_follows_per_day`, default 3) that is **bounded by the account envelope but never in it** — spent on the CLICK (recorded at dispatch) and re-read per target, never decremented from a per-run local. `following` is written ONLY after the control confirms the flip, POLLED (`_await_follow_flip`) because LinkedIn replaces the top card; terminal means no more CLICKS, so a `follow_failed` target is still re-read read-only (`reconcile_roster_follow_state`) and an affirmative "Following" clears it. `FollowStatus` (db.py) is the ONE vocabulary — MySQL ENUM, DOM reading, write sites. The control must NAME the page owner (title-derived, href/label routes, never a class); no owner-named control = `unknown` = click nothing. Full posture: `docs/engagement-automation.md`.
...
- **LEM identity + sessions** (#745 phase 2b): `users.public_uid` is the identity; email is a movable ATTRIBUTE (`change_user_email` + `user_email_history`, PIN to the NEW address, other sessions revoked). `sessions.session_token` stores `SHA-256(token)` — UNKEYED, so a rotated `LEM_SECRET_KEY` never logs everyone out — and it rides in an **httpOnly** cookie. `api/main.get_session_user_id()` is the ONE resolver: an explicit token that RESOLVES wins, else the request's cookie off a ContextVar; the SPA sends the sentinel `'cookie'` (no secret, and no identity — never a cache key). **Since #914 EVERY `/api` route resolves its caller through it** — `require_session_user_id()` is it plus a 401; an `email`/`user_id`/`post_id` is a TARGET to authorise (`_reject_foreign_email` / `_reject_foreign_user_id` / `_require_own_posts` → 403 + an audited `foreign_target_denied`), not the actor; `db.user_owns_posts` FAILS CLOSED and judges a batch whole, and a DB fault is **503** not 403 (an outage ≠ a denial); importing `db.get_session_user_id` around the wrapper is a bug. Sessions are per-device revocable (`GET /user/security`, `/user/sessions/revoke`). Auth limiting is TWO layers: Redis windows (`utilities/auth_rate_limit.py`, **fails open**) + the durable PIN lockout in `verify_pin_for_email` (the hard gate); all lands in `auth_audit_log`. Full: `docs/identity-and-sessions.md`.

...
| **Content-quality telemetry** (#630) | `auto_nightly_content_quality` | The TREND LINE, not a gate — **unscored is never zero**, and it pauses nothing (safety is #629). The three surfaces are NOT one scale (#1433): `similarity_creep` grades the per-surface split (`mix_adjusted_similarity_delta`), never the pooled mean, and no surface has an absolute similarity ceiling here. A DECK is a fourth surface but not a writing one (#1513): `surface="carousel"` carries only the render reading, measured off the receipt the renderer wrote (`utilities/deck_render.py`) because the clip can only be seen while it happens — `deck_probe` `missing`/`unreadable` is NULL dimensions, never 0 | `docs/content-quality-telemetry.md` |

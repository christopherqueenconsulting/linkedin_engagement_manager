...
| **Newsletter covers + publish gate** (#893, #1432, #1135) | `_approved_cover_path`; `get_editions_due_to_publish` | A **generated** cover lands `pending_review`, and an unapproved one is notify-and-publish. A `draft` edition ships only under `auto_publish_newsletters` | `docs/newsletter-covers.md` |
...
| **Occasion / milestone posts** (#1074, #1088) | `OCCASION_FORMAT_KEYS` (one per picker row, #2140); `linkedin/share_composer.py` | The ONLY archetypes nothing picks automatically, enforced by `posts.manual_publish`. A Post click the feed never confirmed is held at `error`, NEVER retried | same |
...
| **Personal-key scoping** (#1453) | `utilities/posthog_keys.py` | The ONE place a personal API key is resolved, BY PURPOSE, each falls back to `POSTHOG_PERSONAL_API_KEY`. 4 of 5 consumers fail SILENTLY; `posthog_key_check.py` proves it | same |

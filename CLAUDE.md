...
| **Weekly group post** (#932, #1224) | `auto_draft_group_post` → `auto_post_to_group` | TWO beats with a review window between. Tue publishes the existing draft and generates NOTHING, so **a run with no READY draft publishes nothing**. Resting status is `ready` so silence ships it; ONE open draft per user, carried forward, never replaced — a restore that would make a SECOND is a 409. The user owns `ready`⟷`skipped` and NOTHING else (`user_settable()`); media rides the post-image surface behind `owns_post_image_url`, goes in BEFORE the text, and fails OPEN. `GROUP_POST_BEST_PRACTICES` is ONE list the prompt follows and the studio shows (`docs/group-posts.md`) |

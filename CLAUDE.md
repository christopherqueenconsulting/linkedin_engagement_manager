...
| **Profile-viewer outreach** (#1137) | `engage_with_profile_viewer` + `profile_viewer_dm_auto_send` | The COLD lane: ONE toggle (default OFF; an unreadable prefs read is OFF) gates BOTH branches — off files PENDING `source='profile_viewer'` rows and dispatches NOTHING. Queued invites SPEND the shared cap |

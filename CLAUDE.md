...
summaries — as a 30-day plan auto-scheduled around golden/peak hours, with self-healing carousels and
...
| **DMs + follow-ups** | `build_dm_from_template`, `dm_templates`, `dm_followups`, `process_user_followups` | Templated and voice-aligned; multi-touch sequences are per-user paced |
...
| **Message-thread ladder** | `linkedin/message_thread.py` (#731) | A route counts only when the thread is **provably open** — never class names, only href/aria-label/TEXT. `ThreadState` is three-valued; **UNKNOWN SKIPS**. Reading a thread and SENDING into one are different questions (#1030): `send_dm_now` NAVIGATES via `open_addressed_composer` and refuses unless `composer_recipient` names someone (`compose_url_for` must carry `recipient=` AND `profileUrn=`). Sent means the message LANDED (`_dm_send_landed`), never that Send took a click |
...
| **Docs surface** (#1020) | `_hide_admin_routes_from_schema()` | `/api/docs`, `/api/redoc`, `/api/openapi.json` (old paths 301). Every `/api/admin/*` operation is kept OUT of the published schema, derived from the route table so a new admin route inherits it. **Hidden ≠ gated** — auth is unchanged, Swagger just can't drive them. The schema is PUBLIC, so `ResponseModel` is generic (#1219): a handler annotates the payload it returns (`ResponseModel[str]` / `[dict[str, Any]]` / `[list[dict[str, Any]]]`) and FastAPI serializes THROUGH it — a named-field model would silently drop keys, so the parameter is always a container type. Bare `ResponseModel` still means `Any` but no operation may reference it. Unauthenticated `GET /health/deep` returns COUNTS only, `"status":"healthy"` first — a monitor contract | same, `docs/stack-watchdog.md` |
...
- **ENUM columns:** `logs.action_type` and other status columns are MySQL ENUMs — adding a value
  ...
  **db-migration** skill and `compose/local/database/migrations/README.md`.

...
| **Docs surface** (#1020) | `_hide_admin_routes_from_schema()` | `/api/docs`, `/api/redoc`, `/api/openapi.json` (old paths 301). Every `/api/admin/*` operation is kept OUT of the published schema, derived from the route table so a new admin route inherits it. **Hidden ≠ gated** — auth is unchanged, Swagger just can't drive them. The schema is PUBLIC, so `ResponseModel[T]` (#1219): each handler names its payload, FastAPI serializes THROUGH it, so `T` is always a CONTAINER type and no operation may `$ref` the bare envelope. Unauthenticated `GET /health/deep` returns COUNTS only, `"status":"healthy"` first — a monitor contract | same, `docs/stack-watchdog.md` |

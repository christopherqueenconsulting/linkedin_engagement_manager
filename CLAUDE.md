...
| **Weekly group post** (#932, #1224) | `auto_draft_group_post` → `auto_post_to_group` | TWO beats, so **a run with no READY draft publishes nothing**. ONE open draft per user, carried forward; a restore making a SECOND is a 409. User owns `ready`⟷`skipped` only; media rides `owns_post_image_url`, goes in BEFORE the text, fails OPEN (`docs/group-posts.md`) |

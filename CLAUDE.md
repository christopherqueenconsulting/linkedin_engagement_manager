...
- **Strong auth + step-up** (#745 phase 2c): `utilities/auth_factors.py` is the ONE place a user's factor state is decided (`webauthn_util.py` holds the ceremonies). Once an account enrols a passkey or TOTP, the email PIN is a **bootstrap** — it mints no session, it mints a pending handle (the no-mail-provider bypass takes the same branch). A **passkey** login is the only phishing-resistant path and the only one that arrives already stepped up; a **recovery code** signs you in but never steps up. `sessions.last_verified_at` gates every credential-touching write (`li_at`, password, email change, session revoke, factor removal, recovery regeneration) — refusal is **403 `step_up_required`**, never 401 (the SPA reads 401 as a dead session). **Adding a factor is never gated, removing one always is**; an account with NO factor passes (opt-in rollout). The extension can't run WebAuthn, so its step-up happens once in the SPA at `/user/extension-token`, which mints a `scope='extension'` session. `STRONG_AUTH_ENABLED=false` reverts to 2b without deleting a factor. Full: `docs/strong-authentication.md`.

...
- **Roster targets LEM can't comment on** (`comment_on_roster_posts` + `auto_follow_roster_target`, issue #962): an author who restricts commenting renders NO comment affordance, so the roster skipped them **invisibly**. A visit that finds posts but zero commentable cards is the restricted-comments signature and records a blocked visit (`record_target_comment_blocked`); "no posts at all" records nothing. The per-visit skip is DEBUG (working behaviour on their side) — only the streak CROSSING the badge threshold logs INFO, once. A landed comment clears the streak in `record_target_engagement`'s own statement. **Auto-follow is opt-in and OFF by default** (`roster_auto_follow`), runs only on the page the roster pass already opened and only AFTER its comment walk, draws its own paced budget (`ACTION_FOLLOW` / `max_follows_per_day`, default 3) that is **bounded by the account envelope but never in it**, and writes `following` ONLY after the control confirms the flip — `following`/`follow_failed` are terminal. The control is scoped to the profile's OWN top card (href/label routes, never a class); both routes missing = `unknown` = click nothing. Full posture: `docs/engagement-automation.md`.

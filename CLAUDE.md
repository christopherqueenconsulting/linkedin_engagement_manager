...
| **Profile-viewer outreach** (#1137) | `engage_with_profile_viewer` + `profile_viewer_dm_auto_send` | The COLD lane, so ONE toggle (default OFF) gates BOTH branches. Off files PENDING `scheduled_dms` / `connection_requests` (`source='profile_viewer'`, #398's table, never a new one) and dispatches NOTHING; an unreadable prefs read is OFF. The COMMENT half is never gated; the ladder starts when the DM LANDS, never at draft time. A queued invite is SPENT `max_invites_per_day` that never ages out, so the queue path is bounded by the budget #486/#979 spend |

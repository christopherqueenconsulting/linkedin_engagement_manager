...
- **Appreciation sources** (#968): recommendation + collaboration read STANDING lists (own profile → Recommendations Received; the mentions notification feed), not event queues, so an **undated card is SKIPPED** (it could be from 2018) and only `APPRECIATION_LOOKBACK_DAYS` (30) counts. `appreciation_touches` is the durable CLAIM that makes this safe — the beat re-queues every ~60s, so without it a standing list is a DM a minute; checked before the message is written, claimed before the send, unreadable ledger = don't send. The ledger stops one person being thanked twice; `_appreciation_dm_budget` (the SHARED `max_dms_per_day` + #626 envelope, computed once and threaded through all three triggers) stops thirty being thanked at once — what it can't afford is left UNCLAIMED for a later pass. Cards that render but NONE of which date is the one thing that warns (silently dead trigger). OFF until live-grounded (`APPRECIATION_SOURCES_ENABLED`, `--appreciation-sources` probe).

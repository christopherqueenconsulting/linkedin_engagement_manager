...
| **Weekly group post** (#932, #1224, #1415) | `auto_draft_group_post` → `auto_post_to_group`; window in `utilities/group_post_slot.py` | TWO beats, so **a run with no READY draft publishes nothing**. ONE open draft per user; a restore making a SECOND is a 409. User owns `ready`⟷`skipped` only; media rides `owns_post_image_url`, goes in BEFORE the text, fails OPEN. Undoing a skip RESTORES that row (never regenerates) and dies at the slot the draft is WAITING ON (first slot after the row was last written — never `created_at` alone, which a carried-forward draft has already passed) — `can_undo_skip` tells the SPA, a late undo is a 409 |

...
- **Unified content core** (`docs/content-core.md`): newsletters, posts AND comments draw framework,
  ...
  degrading to `POST_SIMILARITY_MAX` token overlap, ONE retry then keep, the `similarity` gate naming
  the measure that fired. One measure vocabulary with the nightly telemetry, never two. **Story bank**

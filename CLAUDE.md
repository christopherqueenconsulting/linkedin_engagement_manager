...
- **Occasion / milestone posts** (`project_launch` / `educational_milestone` in `content_framework.py`, #1074, `docs/content-core.md`): LinkedIn's native "Celebrate an occasion" composer has NO API entity, so LEM drafts the copy and the author pastes it in. The two archetypes are the ONLY ones nothing picks automatically (`_rotatable` keeps them out of rotation, variety repair and the planner menu) — a human names a real event or they never appear, and they are absent from the `POST_DAY_TYPES` cadence and the 70/20/10 mix. `posts.manual_publish` is the enforcement: the scheduler's query never returns one and `post_to_linkedin` refuses one that reaches it. `POST /user/post/mark-posted` is the author saying it landed, and is refused for any post that is NOT `manual_publish`.
